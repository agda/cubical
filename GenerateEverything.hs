...
srcDirs        = ["Cubical/HITs", "Cubical/Basics"]

excludedModules :: [FilePath]
excludedModules = map toAgdaFilePath
  [ "Cubical.HITs.HInt"
  , "Cubical.HITs.HitInt"
...
main = do
  ...
  mapM_ (generateFor header) srcDirs

generateFor :: String -> FilePath -> IO ()
generateFor header srcDir = do
  ...
  let fileNoExt    = srcDir ++ "/" ++ allOutputFile

  writeFileUTF8 (fileNoExt ++ ".agda") $
    unlines [ header
            , mkModule $ map (\c -> if c == '/' then '.' else c) fileNoExt
            , format $ filter ((`notElem` excludedModules) . fst) content
            ]
...
usage :: String
usage = unlines
  [ "GenerateEverything: A utility program for Agda's cubical library."
  , ""
  , "Usage: runghc GenerateEverything.hs"
  , ""
...
extractHeader mod = fmap (extract . lines) $ readFileUTF8 mod
  ...
  extract (d1 : "-- Experimental library for Cubical Agda" : "--" : ss)
    | delimiter d1
  ...
  extract _ = [] -- error $ mod ++ " is malformed. It is required to have a module header. Please see other existing files or consult the agda-stdlib authors."
