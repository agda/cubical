...
module Wait (onWait : List ErrorPart  → TC Unit) where
 ...
 wait-for-term t@(con c args) = do
   onWait (strErr "wait-for-con: " ∷ termErr t ∷ [])
   con c <$> wait-for-term-args args
 wait-for-term t@(def f args) = do
   onWait (strErr "wait-for-def: " ∷ termErr t ∷ [])
   def f <$> wait-for-term-args args
 ...
 wait-for-term (meta x x₁) = do
   onWait (strErr "blocked on: " ∷ termErr (meta x x₁) ∷ [])
   blockOnMeta x
...
module PathTypesReflection (onWait : List ErrorPart  → TC Unit) where
 open Wait onWait
...
open Wait (debugPrint "waitForMetas" 20 ) public

open PathTypesReflection (λ _ → pure _) public
