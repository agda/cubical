...
module Cubical.Tactics.GroupSolver.GroupoidSolver where
...
open import Cubical.WildCat.Free3
...
module _ (WG : WildGroupoid ℓg ℓg') (isSetHom : ∀ x y →
      isSet (WildCat.Hom[_,_] (WildGroupoid.wildCat WG) x y) ) where

 open WGE WG isSetHom
 ...
 module Example (x y z w : ob) (p p' : Hom[ x , y ]) (q : Hom[ y , z ])
  ...
  pAe : GrpExpr x w 
  pAe = ([[ p ]]E ⋆E (idE ⋆E [[ q ]]E)) ⋆E invE [[ r ]]E

  pBe : GrpExpr x w 
  pBe = (((([[ p ]]E ⋆E (([[ q ]]E  ⋆E invE (invE [[ r ]]E ⋆E  [[ r ]]E)) ⋆E invE [[ q ]]E))
                  ⋆E ([[ q ]]E ⋆E idE))) ⋆E invE [[ r ]]E) ⋆E idE


  pA=pB : pA ≡ pB
  pA=pB =
    -- _ ≡⟨ sym (GrpExpr→FWG≡WG pAe) ⟩ 
    --     fwgCode''
    --       (GrpExpr→FWG (([[ p ]]E ⋆E (idE ⋆E [[ q ]]E)) ⋆E invE [[ r ]]E))
    --     ≡⟨ {!!} ⟩
    --     fwgCode''
    --       (GrpExpr→FWG
    --        (((([[ p ]]E ⋆E
    --            (([[ q ]]E ⋆E invE (invE [[ r ]]E ⋆E [[ r ]]E)) ⋆E invE [[ q ]]E))
    --           ⋆E ([[ q ]]E ⋆E idE))
    --          ⋆E invE [[ r ]]E)
    --         ⋆E idE))
    --     ≡⟨ GrpExpr→FWG≡WG pBe ⟩
    --     {!!} ∎ 
    sym (GrpExpr→FWG≡WG pAe)
       ∙∙ congS {x = GrpExpr→FWG pAe}
                {y = GrpExpr→FWG pBe}
        (fwgCode'' {x} {w})
          {!!}
        ∙∙ (GrpExpr→FWG≡WG pBe)
