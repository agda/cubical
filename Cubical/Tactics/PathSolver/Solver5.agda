...
module Cubical.Tactics.PathSolver.Solver5 where
...
groupoidSolverTerm debugFlag  inp₀ inpF ty = do

 -- inp₀ ← wait-for-type inp₀'
 
 -- R.typeError (niceAtomList [)
 (A' , (t0' , t1')) ← wait-for-type ty >>= (get-boundaryWithDom ) >>= Mb.rec
 ...
 (r0 , r0') ← tryPathE.try≡ inp [t] [f] 100 t0
 (r1 , r1') ← tryPathE.try≡ inp [t] [f] 100 t1
...
groupoidSolverMain debugFlag inp inpF hole = do
  ...
  (solution , msg) ← groupoidSolverTerm debugFlag inp inpF ty
  R.catchTC
...
groupoidSolverMainTerm' : Bool → R.Term → R.Term → R.TC Unit
groupoidSolverMainTerm' debugFlag ty hole = do
  ...
  -- R.commitTC
  (solution , msg) ←  groupoidSolverTerm debugFlag inp inpF ty
  solutionR ← R.quoteTC solution 
  R.catchTC
    (R.unify hole solutionR)
    (R.typeError msg)
...
macro
 ...
 ≡!trm : R.Term → R.Term → R.TC Unit
 ≡!trm = groupoidSolverMainTerm' true
