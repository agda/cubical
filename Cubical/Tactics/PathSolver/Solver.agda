...
module Cubical.Tactics.PathSolver.Solver where
...
typeView : R.Term → R.TC (Maybe R.Term)
typeView t =
  ⦇ just (R.checkType t (R.def (quote Type) v[ R.unknown ])) ⦈ <|> ⦇ nothing ⦈



-- macro
--  tvTest : R.Term → R.Term → R.TC Unit
--  tvTest ty? hole =
--    typeView ty? >>=
--      Mb.caseMaybe
--       (R.typeError [ "not a type!" ]ₑ)
--       (R.typeError [ "is type!" ]ₑ)
...
data [Fns] : Typeω where
 [fn] : [Fns]
 _∷_ : ∀ {ℓ ℓ'} → {A : Type ℓ} {A' : Type ℓ'} → (A → A') → [Fns] → [Fns]


gatherCodoms : R.Term → R.TC (List R.Term)
gatherCodoms t@(R.con (quote [fn]) _) = pure []
gatherCodoms t@(R.con (quote [Fns]._∷_) (_ h∷ _ h∷ a' h∷ _ h∷ _ v∷ xs v∷ [])) =
  ⦇ ⦇ a' ⦈ ∷ gatherCodoms xs ⦈
gatherCodoms t = R.typeError $  "gatherCodoms - FAIL : " ∷ₑ [ t  ]ₑ

inferTypesFromFns : R.Term → R.TC (List R.Term)
inferTypesFromFns =
  (wait-for-term >=> gatherCodoms) >=> uniqeAtoms
...
reflected[Typeₙ]→[reflectedTy] (R.con (quote _∷Tω_) (x v∷ x₁ v∷ [])) =
  (x ∷_) <$> reflected[Typeₙ]→[reflectedTy] x₁
reflected[Typeₙ]→[reflectedTy] _ = R.typeError [ "reflected[Typeₙ]→[reflectedTy] - FAIL" ]ₑ
 -- typeView x >>=
 --     Mb.rec
 --      (R.typeError [ "reflected[Typeₙ]→[reflectedTy] - FAIL" ]ₑ)
 --      (pure ∘ [_])

[reflectedTy]→reflected[Typeₙ] : List R.Term → R.Term
[reflectedTy]→reflected[Typeₙ] [] = (R.con (quote [Tω]) [])
[reflectedTy]→reflected[Typeₙ] (x ∷ xs) =
 (R.con (quote _∷Tω_) (x v∷ [reflectedTy]→reflected[Typeₙ] xs v∷ []))

castTo[Typeₙ] : R.Term → R.TC (R.Term)
castTo[Typeₙ] t@(R.con (quote [Tω]) _) = pure t
castTo[Typeₙ] t@(R.con (quote _∷Tω_) _) = pure t
castTo[Typeₙ] x =
 typeView x >>=
     Mb.rec
      (R.typeError [ "castTo[Typeₙ] - FAIL" ]ₑ)
      (λ t → pure (R.con (quote _∷Tω_) (t v∷ (R.con (quote [Tω]) []) v∷ [])))

castTo[Fns] : R.Term → R.TC (R.Term)
castTo[Fns] t@(R.con (quote [fn]) _) = pure t
castTo[Fns] t@(R.con (quote [Fns]._∷_) _) = pure t
castTo[Fns] x =
    R.checkType
     (R.con (quote [Fns]._∷_) (x v∷ (R.con (quote [fn]) []) v∷ []))
     (R.def (quote [Fns]) [])
 -- funView x >>=
 --     Mb.rec
 --      (R.typeError [ "castTo[Fns] - FAIL" ]ₑ)
 --      (λ t → pure (R.con (quote [Fns]._∷_) (t v∷ (R.con (quote [fn]) []) v∷ [])))
...
reflected[Fns]→[reflectedFn] (R.con (quote [Fns]._∷_) (_ h∷ _ h∷ _ h∷ _ h∷ x v∷ x₁ v∷ [])) = (x ∷_) <$> reflected[Fns]→[reflectedFn] x₁
reflected[Fns]→[reflectedFn] (R.con (quote [Fns]._∷_) (_ h∷ _ h∷ _ h∷ x v∷ x₁ v∷ [])) =
  (x ∷_) <$> reflected[Fns]→[reflectedFn] x₁
reflected[Fns]→[reflectedFn] (R.con (quote [Fns]._∷_) (_ h∷ _ h∷ x v∷ x₁ v∷ [])) =
  (x ∷_) <$> reflected[Fns]→[reflectedFn] x₁
reflected[Fns]→[reflectedFn] (R.con (quote [Fns]._∷_) (_ h∷ x v∷ x₁ v∷ [])) =
  (x ∷_) <$> reflected[Fns]→[reflectedFn] x₁
reflected[Fns]→[reflectedFn] (R.con (quote [Fns]._∷_) (x v∷ x₁ v∷ [])) =
  (x ∷_) <$> reflected[Fns]→[reflectedFn] x₁
...
module tryTermE ([TC[T]trm] [fns] : List R.Term) where
...
groupoidSolverTerm debugFlag  inpF hole = do

 -- inp₀ ← wait-for-type inp₀'

 -- R.typeError (niceAtomList [)     -- >>= wait-for-type
 (A' , (t0' , t1')) ← R.inferType hole  >>= (get-boundaryWithDom ) >>= Mb.rec
 ...
 (AA , _) ← get-boundaryWithDom A' >>= Mb.rec
     ...
     (λ x → R.returnTC x)
 inp₀ ← ([reflectedTy]→reflected[Typeₙ] <$> inferTypesFromFns inpF) >>= wait-for-term
...
groupoidSolverMain debugFlag inpF' hole = do
  inpF ← castTo[Fns] inpF'
  inp ← inferTypesFromFns inpF
  R.commitTC
  ty ← R.withNormalisation true $  R.inferType hole >>= wait-for-term
  hole' ← R.withNormalisation true $ R.checkType hole ty

  (solution , msg) ← groupoidSolverTerm debugFlag inpF hole'
  R.catchTC
   (R.unify hole solution)
   (R.typeError msg)

squareSolverMain : Bool → R.Term → R.Term → R.TC Unit
squareSolverMain debugFlag inpF' hole = do
  inpF ← castTo[Fns] inpF'
  inp ← inferTypesFromFns inpF
  ty ← R.inferType hole >>= wait-for-term
...
macro
 ...
 ≡! : R.Term → R.TC Unit
 ≡! = groupoidSolverMain true (R.con (quote [fn]) [])

 ≡!cong : R.Term → R.Term → R.TC Unit
 ≡!cong = groupoidSolverMain true


 sq! : R.Term → R.TC Unit
 sq! = squareSolverMain false (R.con (quote [fn]) [])

 sq!cong : R.Term → R.Term → R.TC Unit
 sq!cong = squareSolverMain true
