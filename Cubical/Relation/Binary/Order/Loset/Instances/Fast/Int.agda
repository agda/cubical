module Cubical.Relation.Binary.Order.Loset.Instances.Fast.Int where
...
isLoset (snd ℤ<Loset) = isLosetℤ<
  where
    ...
    abstract
      ...
      isLosetℤ< .is-asym        = λ _ _ a<b b<a → isIrrefl< (isTrans< a<b b<a)
      isLosetℤ< .is-weakly-linear a b c a<b with a ≟ c
      ... | lt a<c = ∣ inl a<c ∣₁
      ... | eq a≡c = ∣ inr (recompute< (subst (_<ℤ b) a≡c a<b)) ∣₁
