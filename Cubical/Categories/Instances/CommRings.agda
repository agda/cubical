...
module PreSheafFromUniversalProp (C : Category ℓ ℓ') (P : ob C → Type ℓ)
         (𝓕 : Σ (ob C) P → CommRing ℓ)
         (uniqueHom : ∀ x y → C [ fst x , fst y ] → isContr (CommRingHom (𝓕 y) (𝓕 x)))
         where
 ...
 𝓕UniqueEquiv : (x : ob C) (p q : P x) → isContr (CommRingEquiv (𝓕 (x , p)) (𝓕 (x , q)))
 𝓕UniqueEquiv x = contrCommHom→contrCommEquiv (curry 𝓕 x) λ p q → uniqueHom _ _ (id C)

 theMap : (x : ob C) → ∥ P x ∥ → CommRing ℓ
 theMap x = recPT→CommRing (curry 𝓕 x) (λ p q → 𝓕UniqueEquiv x p q .fst)
                                         λ p q r → 𝓕UniqueEquiv x p r .snd _

 theAction : (x y : ob C) → C [ x , y ]
           → (p : ∥ P x ∥) (q : ∥ P y ∥) → isContr (CommRingHom (theMap y q) (theMap x p))
