...
module _ {ℓ ℓ'} (A : Pointed ℓ) (B : Pointed ℓ') where
  ...
  test : Iso (coHom 1 (A ⋁ B)) (fst (×coHomGr 1 (typ A) (typ B)))
  Iso.fun test =
    sElim (λ _ → isSet× setTruncIsSet setTruncIsSet)
      λ f → ∣ (λ x → f (inl x)) ∣₂ , ∣ (λ x → f (inr x)) ∣₂
  Iso.inv test =
    uncurry (sElim2 (λ _ _ → setTruncIsSet)
                    λ f g → ∣ (λ {(inl x) → f x +ₖ g (pt B)
                                ; (inr x) → g x +ₖ f (pt A)
                                ; (push tt i) → commₖ 1 (f (pt A)) (g (pt B)) i}) ∣₂)
  Iso.rightInv test =
    uncurry
  ...
  Iso.leftInv test = {!!}
    where
    helper : (f : A ⋁ B → coHomK 1) (x y : coHomK 1)
          → f (inl (pt A)) ≡ x → f (inr (pt B)) ≡ y
          → ∥ f ≡ (λ {(inl x) → f (inl x) +ₖ f (inr (pt B))
                     ; (inr x) → f (inr x) +ₖ f (inl (pt A))
                     ; (push tt i) → commₖ 1 (f (inl (pt A))) (f (inr (pt B))) i}) ∥
    helper f =
      trElim2 {!!}
        (sphereElim2 {!!} {!!}
          λ inlId inrId → trRec {!!} (λ _ → ∣ funExt (λ { (inl x) → sym (rUnitₖ 1 (f (inl x))) ∙ cong (f (inl x) +ₖ_) (sym inrId)
                                                         ; (inr x) → sym (rUnitₖ 1 (f (inr x))) ∙ cong (f (inr x) +ₖ_) (sym inlId)
                                                         ; (push tt i) → {!!}}) ∣₁)
                                      (isConnectedPathKn 0 (f (inl (pt A))) {!!} .fst))
