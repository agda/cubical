...
coHom1S1-helper : Iso (coHom 2 (S₊ 1)) Unit
coHom1S1-helper =
  compIso (setTruncIso test13)
    (compIso
      setTruncOfProdIso
      (compIso
  ...
  Iso'→Iso
    (iso'
      (iso
        ...
        (λ _ → 0ₕ)
        (λ _ → refl)
        (λ _ → isPropcoHom1S1 _ _))
      λ _ _ → refl)
  where
  isPropcoHom1S1 : isProp (coHom 2 (S₊ 1))
  isPropcoHom1S1 =
    isOfHLevelSuc 0
      (transport (λ i → isContr (isoToPath coHom1S1-helper (~ i)))
                 isContrUnit)
