...
basechange2⁻Gen : (x y : S¹) → x ≡ y → x ≡ y → ΩS¹
basechange2⁻Gen x y = J (λ y p → x ≡ y → ΩS¹) (basechange2⁻ x)

basechange2⁻Gen-morphIsh : (x y z : S¹) → (P : x ≡ y) (Q : y ≡ z) (p : x ≡ y) (q : y ≡ z) → basechange2⁻Gen x z (P ∙ Q) (p ∙ q) ≡ basechange2⁻Gen x y P p ∙ basechange2⁻Gen y z Q q
basechange2⁻Gen-morphIsh x y z =
  J (λ y P → (Q : y ≡ z) (p : x ≡ y) (q : y ≡ z) → basechange2⁻Gen x z (P ∙ Q) (p ∙ q) ≡ basechange2⁻Gen x y P p ∙ basechange2⁻Gen y z Q q)
    (J (λ z Q → (p : x ≡ x) (q : x ≡ z) → basechange2⁻Gen x z (refl ∙ Q) (p ∙ q) ≡ basechange2⁻Gen x x refl p ∙ basechange2⁻Gen x z Q q)
       λ p q → cong (λ y → basechange2⁻Gen x x y (p ∙ q)) (sym (lUnit refl))
              ∙ (λ i → JRefl (λ y p → x ≡ y → ΩS¹) (basechange2⁻ x) i (p ∙ q))
              ∙ basechange2⁻-morph x p q
              ∙ λ i → JRefl (λ y p → x ≡ y → ΩS¹) (basechange2⁻ x) (~ i) p ∙ JRefl (λ y p → x ≡ y → ΩS¹) (basechange2⁻ x) (~ i) q)
...
Iso.rightInv S¹→S¹≡S1→S1 F = funExt λ x i → helper (F (S1→S¹-sect x i)) i
  where
  helper : (x : hLevelTrunc 3 S¹) → trMap S1→S¹ (trMap S¹→S1 x) ≡ x
  helper = trElim (λ _ → isOfHLevelPath 3 (isOfHLevelTrunc 3) _ _)
                  λ a → cong ∣_∣ (S1→S¹-sect a)
Iso.leftInv S¹→S¹≡S1→S1 F = funExt λ x i → helper (F (S1→S¹-retr x i)) i
  where
  helper : (x : hLevelTrunc 3 (S₊ 1)) → trMap S¹→S1 (trMap S1→S¹ x) ≡ x
  helper = trElim (λ _ → isOfHLevelPath 3 (isOfHLevelTrunc 3) _ _)
      ...
      (iso
        ...
        (sElim (λ _ → isOfHLevelPath 2 setTruncIsSet _ _)
               λ f → (λ i → ∣ Iso.inv S¹→S¹≡S1→S1 (Iso.inv S¹→S¹ (base , proj₂ (Iso.fun S¹→S¹ (Iso.fun S¹→S¹≡S1→S1 f)))) ∣₀)
                    ∙ (λ i → ∣ Iso.inv S¹→S¹≡S1→S1 {!!} ∣₀)
                    ∙ {!!}
                    ∙ {!!}
                    ∙ {!!}))
        {!!})
    where
    helper : (f : _) (x : S¹) → ∣ (Iso.inv S¹→S¹≡S1→S1 (Iso.inv S¹→S¹ (base , proj₂ (Iso.fun S¹→S¹ (Iso.fun S¹→S¹≡S1→S1 f))))) ∣₀ ≡ ∣ (Iso.inv S¹→S¹≡S1→S1 (Iso.inv S¹→S¹ (Iso.fun S¹→S¹ (Iso.fun S¹→S¹≡S1→S1 f)))) ∣₀
    helper f x = sym ({!!}
                      {!!} ∙
                      {!!} ∙
                      {!!} ∙
...
{-
  ...
  helper : (x y : hLevelTrunc 3 (S₊ 1)) (f g : S₊ 1 → ∥ S₊ 1 ∥ ℕ→ℕ₋₂ 1)
        → (f (S¹→S1 base)) ≡ x
        → (g (S¹→S1 base)) ≡ y
...
{-
Goal: basechange2⁻
      (S¹map (trMap S1→S¹ (ΩKn+1→Kn (Kn→ΩKn+1 1 (f (S¹→S1 base))))))
      (cong
       (λ x → S¹map (trMap S1→S¹ (ΩKn+1→Kn (Kn→ΩKn+1 1 (f (S¹→S1 x))))))
       loop)
      ∙
      basechange2⁻
      (S¹map (trMap S1→S¹ (ΩKn+1→Kn (Kn→ΩKn+1 1 (g (S¹→S1 base))))))
      (cong
       (λ x → S¹map (trMap S1→S¹ (ΩKn+1→Kn (Kn→ΩKn+1 1 (g (S¹→S1 x))))))
       loop)
      ≡
      basechange2⁻ (S¹map (trMap S1→S¹ (f (S¹→S1 base))))
      (λ i → S¹map (trMap S1→S¹ (f (S¹→S1 (loop i)))))
      ...
      (λ i → S¹map (trMap S1→S¹ (g (S¹→S1 (loop i)))))
-}
-- (f g : S₊ 1 → ∥ S₊ 1 ∥ ℕ→ℕ₋₂ 1) → (f (S¹→S1 base)) ≡ ∣ x ∣ → (g (S¹→S1 base)) ≡ ∣ y ∣ → ?
    ...
    (iso'
      (iso
        ...
        (sElim2 (λ _ _ → isOfHLevelPath 2 isSetInt _ _)
                λ f g → (λ i → winding (basechange2⁻ (S¹map (trMap S1→S¹ (ΩKn+1→Kn (Kn→ΩKn+1 1 (f (S¹→S1 base)) ∙ Kn→ΩKn+1 1 (g (S¹→S1 base))))))
                                                (λ i → S¹map (trMap S1→S¹ ((ΩKn+1→Kn (Kn→ΩKn+1 1 (f (S¹→S1 (loop i))) ∙ Kn→ΩKn+1 1 (g (S¹→S1 (loop i))))))))))
                       ∙ (λ j → winding (basechange2⁻ (S¹map (trMap S1→S¹ (ΩKn+1→Kn {!basechange2⁻!})))
                                                (λ i → S¹map (trMap S1→S¹ (ΩKn+1→Kn {!!})))))
                       ∙ {!!}
                       ∙ {!λ i → winding !}
                       ∙ winding-hom (basechange2⁻ (S¹map (trMap S1→S¹ (f (S¹→S1 base))))
                                                   (λ i → S¹map (trMap S1→S¹ (f (S¹→S1 (loop i)))))) 
                                     (basechange2⁻ (S¹map (trMap S1→S¹ (g (S¹→S1 base))))
                                                   (λ i → S¹map (trMap S1→S¹ (g (S¹→S1 (loop i)))))) ))  -- sym (winding-hom _ _)))
  where
  helper : {!!}
  helper = {!!} -}
