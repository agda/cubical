...
--   niceHelper : (n : ℕ) (h l : C → coHomK n) (x : D .fst) → (d-pre1 n h x +ₖ d-pre1 n l x) ≡ d-pre1 n (λ x → h x +ₖ l x) x
--   niceHelper zero h l (inl x) = {!!}
--   niceHelper zero h l (inr x) = {!!}
--   niceHelper zero h l (push a i₁) = {!!}
--   niceHelper (suc n) h l (inl x) = {!!} -- lUnitₖ 0ₖ
--   niceHelper (suc n) h l (inr x) = {!!} -- lUnitₖ 0ₖ
--   niceHelper (suc n) h l (push a i) j =
...
--   helper h l (push a i) = {!!}
--     where
--     helper1 : Kn→ΩKn+1 zero (h a +ₖ l a) ≡ cong₂ (λ x y → x +ₖ y) (Kn→ΩKn+1 zero (h a)) (Kn→ΩKn+1 zero (l a))
--     helper1 =  (λ i → Kn→ΩKn+1 zero (h a +ₖ l a)) ∙ +ₖ→∙ zero (h a) (l a) ∙
--                {!!} ∙
--                sym (cong₂Funct (λ x y → x +ₖ y) (Kn→ΩKn+1 zero (h a)) (Kn→ΩKn+1 zero (l a))) ∙
--                (λ i j → Kn→ΩKn+1 zero (h a) j +ₖ Kn→ΩKn+1 zero (l a) j) ∙
--                λ i j → ΩKn+1→Kn (Kn→ΩKn+1 1 (Kn→ΩKn+1 zero (h a) j) ∙ Kn→ΩKn+1 1 (Kn→ΩKn+1 zero (l a) j))
--     helper2 : PathP (λ i → rUnitₖ 0ₖ i ≡ rUnitₖ 0ₖ i ) (cong (λ x → x +ₖ ∣ north ∣) (Kn→ΩKn+1 zero (h a)) ∙  cong (λ y → ∣ north ∣ +ₖ y) (Kn→ΩKn+1 zero (l a)))
--                          (Kn→ΩKn+1 zero (h a) ∙ Kn→ΩKn+1 zero (l a))
--     helper2 j i = hcomp (λ k → λ { (i = i0) → rUnitₖ ∣ north ∣ j
--                                   ; (i = i1) → {!!}})
--                         (rUnitₖ (Kn→ΩKn+1 zero (h a) i) j) --  (cong (λ x → {!rUnitₖ {n = 1} x i!}) (Kn→ΩKn+1 zero (h a)) ∙  cong (λ y → lUnitₖ {n = 1} y i) (Kn→ΩKn+1 zero (l a)))

--     helper3 : PathP (λ i → rUnitₖ 0ₖ i ≡ rUnitₖ 0ₖ i) (cong (λ x → x +ₖ ∣ north ∣) (Kn→ΩKn+1 zero (h a))) (Kn→ΩKn+1 zero (h a))
--     helper3 i = cong (λ x → rUnitₖ x i) (Kn→ΩKn+1 zero (h a))

--     heler4 : PathP (λ i → lUnitₖ 0ₖ i ≡ lUnitₖ 0ₖ i) (cong (λ x → ∣ north ∣ +ₖ x) (Kn→ΩKn+1 zero (l a))) (Kn→ΩKn+1 zero (l a))
--     heler4 i = cong (λ x → lUnitₖ x i) (Kn→ΩKn+1 zero (l a))

-- ttest : {!!}
-- ttest = congFunct
--   --   test2 : (n : ℕ) (a : C) (h l : C → coHomK n) → cong₂ (λ x y → x +ₖ y) (Kn→ΩKn+1 n (h a)) (Kn→ΩKn+1 n (l a)) ≡ {!!} ∙ Kn→ΩKn+1 n (h a +ₖ l a) ∙ {!!}
--   --   test2 = {!!}
