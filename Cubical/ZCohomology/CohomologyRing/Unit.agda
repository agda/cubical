...
module Cubical.ZCohomology.CohomologyRing.Unit where
...
e-sect = DS-Ind-Prop.f _ _ _ _ (λ _ → isSetH* _ _ )
  ...
  eq-base zero =  sElim (λ x → isSet→isGroupoid isSetH* (ℤ→H*-Unit (H*-Unit→ℤ (base zero x))) (base zero x))
                  λ a → cong {ℓ-zero} {coHom 0 Unit} {ℓ-zero} {λ _ → H* Unit} -- ask anders 
                              (base 0) (cong ∣_∣₂ (funExt (λ {tt → refl}))) 
  eq-base (suc n) a = base-neutral 0 ∙ sym (base-neutral (suc n))
                      ∙ cong {ℓ-zero} {coHom (suc n) Unit} {ℓ-zero} {λ _ → H* Unit} -- ask anders, same issue 
                        (base (suc n)) (isContr→isProp (isContrHⁿ-Unit n) _ a)
  -- issue : goal inference and type checking are not linked
  --         and so when it type check it doesn't know which base is that
