...
rightInv (suspFunCharac {A = A} n) =
  sElim (λ _ → isOfHLevelPath 2 setTruncIsSet _ _)
        λ f → trRec (isProp→isOfHLevelSuc n (setTruncIsSet _ _))
                (λ fId → cong ∣_∣₂
                (funExt (λ x → (λ i → ΩKn+1→Kn (suc n) (sym (rCancel≡refl n i) ∙∙ cong (λ x → suspΩFun (suc n) f x +ₖ 0ₖ _)
                                                               (merid x ∙ sym (merid (pt A))) ∙∙ rCancel≡refl n i))
                             ∙∙ (λ i → ΩKn+1→Kn (suc n) (rUnit (cong (λ x → suspΩFun (suc n) f x +ₖ 0ₖ _)
                                                                       ((merid x) ∙ sym (merid (pt A)))) (~ i)))
                             ∙∙ (λ i → ΩKn+1→Kn (suc n) (congFunct (λ x → rUnitₖ _ (suspΩFun (suc n) f x) i)
                                                                     (merid x) (sym (merid (pt A))) i))
