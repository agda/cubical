...
module Cubical.ZCohomology.dirrAdd.Groups.Torus where
...
open import Cubical.ZCohomology.dirrAdd.Groups.Prelims
...
test : GroupEquiv (coHomGr 2 (S₊ 1 × S₊ 1)) (coHomGr 2 (S₊ 2))
fst (GroupEquiv.eq test) = sMap λ f → λ {north → f (base , base) ; south → f (base , base) ; (merid a i) → f (base , (loop i))}
equiv-proof (snd (GroupEquiv.eq test)) =
  coHomPointedElim _ north (λ _ → isPropIsContr) λ f fId → (∣ (λ {(_ , base) → f north ; (_ , (loop i)) → f (S²→SuspS¹ (surf i i)) }) ∣₂ , cong ∣_∣₂ (funExt λ {north → refl ; south → cong f (merid base) ; (merid base i) j → {!!} ; (merid (loop j) i) k → {!!}}))
    , uncurry (coHomPointedElim _ (base , base) (λ _ → isPropΠ λ _ → isSetΣ setTruncIsSet (λ _ → isOfHLevelPath 2 setTruncIsSet _ _) _ _)
                                  λ g gId gId2 → Σ≡Prop (λ _ → setTruncIsSet _ _) {!!})
GroupEquiv.isHom test = {!!}
