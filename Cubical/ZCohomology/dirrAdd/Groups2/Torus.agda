...
module Cubical.ZCohomology.Groups2.Torus where
...
H⁰-T²≅ℤ =
  H⁰-connected (base , base)
    (isOfHLevelRetractFromIso 0 (truncOfProdIso 2)
    (isOfHLevelΣ 0 (sphereConnected 1) λ _ → sphereConnected 1))

--------- H¹(T²) -------------------------------

H¹-T²≅ℤ×ℤ : GroupIso (coHomGr 1 ((S₊ 1) × (S₊ 1))) (dirProd intGroup intGroup)
H¹-T²≅ℤ×ℤ = {!Iso.fun test2 (Iso.inv test2 (0 , 0))!} □ dirProdGroupIso (invGroupIso (Hⁿ-Sⁿ≅ℤ 0)) (H⁰-Sⁿ≅ℤ 0) -- theIso □ dirProdGroupIso (invGroupIso (Hⁿ-Sⁿ≅ℤ 0)) (H⁰-Sⁿ≅ℤ 0)
  where
  helper : (x : hLevelTrunc 3 (S₊ 1)) → ∣ (S¹map x) ∣ ≡ x
  helper = trElim (λ _ → isOfHLevelPath 3 (isOfHLevelTrunc 3) _ _)
                  λ a → refl

  typIso : Iso (coHom' 1 (S¹ × S¹)) (coHom' 1 S¹ × coHom' 0 S¹)
  typIso = setTruncIso (curryIso ⋄ codomainIso nice!TOTAL ⋄ toProdIso ⋄ prodIso idIso (codomainIso (invIso IsoLoopK₀Int)))
         ⋄ setTruncOfProdIso

  test2 : Iso _ _
  test2 = compIso typIso (GroupIso→Iso (dirProdGroupIso (invGroupIso (Hⁿ-Sⁿ≅ℤ 0)) (H⁰-Sⁿ≅ℤ 0)))
