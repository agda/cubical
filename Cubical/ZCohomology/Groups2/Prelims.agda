...
maybe : (x : loopK 1) → (x ≡ x) ≃ typ (Ω ((loopK 1) , ∣ refl ∣))
maybe = Iso.inv (elim.isIsoPrecompose {A = Unit} (λ _ → ∣ refl ∣) 2
                                      (λ x → ((x ≡ x) ≃ typ (Ω ((loopK 1) , ∣ refl ∣))) , isOfHLevel≃ 2 {A = x ≡ x} (isOfHLevelTrunc 3 _ _) (isOfHLevelTrunc 3 _ _))
                                      {!!})
                                      {!!}


killSecond : Iso (Σ[ x ∈ loopK 1 ] x ≡ x) (Σ[ x ∈ (loopK 1 × loopK 1) ] fst x ≡ snd x)
killSecond = {!isOfHLevel≃!}
