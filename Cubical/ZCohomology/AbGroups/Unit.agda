{-# OPTIONS --safe #-}
module Cubical.ZCohomology.AbGroups.Unit where
...
open import Cubical.Algebra.Group renaming (Unit to UnitGroup) hiding (ℤ)
open import Cubical.Algebra.AbGroup renaming (trivialAbGroup to UnitAbGroup-ℓ)
open import Cubical.Algebra.Ring
open import Cubical.Algebra.CommRing
open import Cubical.Algebra.CommRing.Instances.Int renaming (ℤ to ℤCR)

ℤAbG = Ring→AbGroup (CommRing→Ring ℤCR)
UnitAbGroup = UnitAbGroup-ℓ {ℓ-zero}
...
Hⁿ-Unit≅0 : (n : ℕ) → AbGroupIso (coHomGroup (suc n) Unit) UnitAbGroup
...
--  Computation of Hⁿ(A) for A contractible
...
Hⁿ-contrType≅0 : ∀ {ℓ} {A : Type ℓ} (n : ℕ) → isContr A
              → AbGroupIso (coHomGroup (suc n) A) UnitAbGroup
