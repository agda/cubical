...
testfun : (n : ℕ) → (hLevelTrunc n (typ (Ω (S₊ (suc n) , north)))) → (typ (Ω (hLevelTrunc (suc n) (S₊ (suc n)) , ∣ north ∣))) 
testfun n = trRec (isOfHLevelPath' n (isOfHLevelTrunc _) _ _)
                         λ a → cong ∣_∣ a

code1 : (n : ℕ) (s : S₊ (suc n)) → (hLevelTrunc n (north ≡ s)) → Path (hLevelTrunc (suc n) (S₊ (suc n))) ∣ north ∣ ∣ s ∣ -- (typ (Ω (hLevelTrunc (suc n) (S₊ (suc n)) , ∣ north ∣))) 
code1 n s = trRec (isOfHLevelPath' n (isOfHLevelTrunc _) _ _)
                         λ a → cong ∣_∣ a

decode1 : (n : ℕ) (s : S₊ (suc n)) → Path (hLevelTrunc (suc n) (S₊ (suc n))) ∣ north ∣ ∣ s ∣ → hLevelTrunc n (north ≡ s)
decode1 = {!!}

isCon-testfun2 : (n : ℕ) (t : S₊ (suc n)) → isHLevelConnectedFun n (code1 n t)
isCon-testfun2 n t = elim.isConnectedPrecompose (code1 n t) n λ P → (λ f → {!P!}) , {!!}
  where
  module _ {ℓ'' : Level} (P : Path (hLevelTrunc (suc n) (S₊ (suc n))) ∣ north ∣ ∣ t ∣ → HLevel ℓ'' n) (F : (a : hLevelTrunc n (north ≡ t)) → P (code1 n t a) .fst) where
    F' : (y : S₊ (suc n)) → _
    F' y = λ(s : (a : _) → P {!a!} .fst) → s ∘ {!code1 n s!}
    
    Q : {!a!}
    Q = {!!}

isCon-testfun : (n : ℕ) → isHLevelConnectedFun n (testfun n)
isCon-testfun n = elim.isConnectedPrecompose (testfun n) n λ P → (λ f → {!!}) , {!!}
  where
  module _ {ℓ'' : Level} (P : typ (Ω (hLevelTrunc (suc n) (S₊ (suc n)) , ∣ north ∣)) → HLevel ℓ'' n) (F : (a : hLevelTrunc n (typ (Ω (S₊ (suc n) , north)))) → P (testfun n a) .fst) where
    F' : _
    F' = λ(s : (a : _) → P {!a!} .fst) → s ∘ testfun n
...
test : (n : ℕ) → Iso (hLevelTrunc n (typ (Ω (S₊ (suc n) , north))))
                       (typ (Ω (hLevelTrunc (suc n) (S₊ (suc n)) , ∣ north ∣))) 
Iso.fun (test n) = trRec (isOfHLevelPath' n (isOfHLevelTrunc _) _ _)
                         λ a → cong ∣_∣ a
Iso.inv (test n) = {!!}
  where
  map2 : hLevelTrunc (suc n) (S₊ (suc n)) → hLevelTrunc n (typ (Ω (S₊ (suc n) , north)))
  map2 = trRec {!!} λ a → ∣ {!a!} ∙ {! →
         HLevel ℓ''' n) →!} ∣
    where
    map3 : (n : ℕ) → S₊ (suc n) → hLevelTrunc n (typ (Ω (S₊ (suc n) , north)))
    map3 zero x = {!!}
    map3 (suc n) = {!!} --  trRec ? ? -- (λ p → ∣ cong (λ x → map4 n x) p ∣) (map3 n (map5 _ x))
      where
      map4 : (n : ℕ) → S₊ (suc n) → S₊ (suc (suc n))
      map4 n north = north
      map4 n south = south
      map4 n (merid a i) = merid (merid a i) i

      map6 : S₊ (suc (suc (suc n))) → typ (Ω (S₊ (suc (suc n)) , north))
      map6 north = {!!}
      map6 south = {!!}
      map6 (merid north i) = merid (merid north i) ∙ sym (merid north)
      map6 (merid south i) = merid (merid south i) ∙ sym (merid north)
      map6 (merid (merid a i₁) i) = merid {!a!} ∙ sym (merid north)

      map5 : (n : ℕ) → S₊ (suc (suc n)) → S₊ (suc n)
      map5 zero north = north
      map5 zero south = south
      map5 zero (merid a i) = {!a!}
      map5 (suc zero) north = north
      map5 (suc zero) south = south
      map5 (suc zero) (merid a i) = merid (map5 zero a) i
      map5 (suc (suc n)) north = north
      map5 (suc (suc n)) south = south
      map5 (suc (suc n)) (merid a i) = merid (map5 (suc n) a) i
Iso.rightInv (test n) = {!!}
Iso.leftInv (test n) = {!!}
...
-- -- We need ΩTrunc. It computes horribly, so its better to restate the function involved for this particular case --
