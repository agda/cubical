...
∪-help : (n m : ℕ) → (S₊ (suc n) → (S₊ (suc m)) → coHomK (suc n +nice suc m))
...
∪-help (suc zero) (suc m) (merid a i) (merid b j) = (sym (Kn→ΩKn+10ₖ _) ∙∙ cong (Kn→ΩKn+1 _) (Kn→ΩKn+1 _ (∪-help zero m a b)) ∙∙ Kn→ΩKn+10ₖ _) i j
∪-help (suc (suc n)) (suc m) (merid a i) (merid b j) = {!(sym (Kn→ΩKn+10ₖ _) ∙∙ cong (Kn→ΩKn+1 _) (Kn→ΩKn+1 _ (∪-help (suc n) m a b)) ∙∙ Kn→ΩKn+10ₖ _) i j!}
{-
  {!Kn→ΩKn+1 _ (∪-help n m a b) -- (sym (Kn→ΩKn+10ₖ _) ∙∙ cong (Kn→ΩKn+1 _) (Kn→ΩKn+1 _ (∪-help n m a b)) ∙∙ Kn→ΩKn+10ₖ _) i j!} -} -- (sym (Kn→ΩKn+10ₖ _) ∙∙ cong (Kn→ΩKn+1 _) (Kn→ΩKn+1 _ (subst coHomK (sym (+-suc n (suc m))) (∪-help n m a b))) ∙∙ Kn→ΩKn+10ₖ _) i j
∪-help-pt zero a = refl
...
--   where
--   congMinner : {k : ℕ} (n m : ℕ) (x : coHomK (suc k)) → cong (minner (suc n) (suc m)) (Kn→ΩKn+1 (suc k) x) ≡ (Kn→ΩKn+1 (suc k) (minner (suc n) (suc m) x))
--   congMinner n m = {!!}
--   congcongMinner : {k : ℕ} (n m : ℕ) (p : coHomK (suc (suc k)))
--                  → cong (cong (minner (suc n) (suc m))) (cong (Kn→ΩKn+1 (3 + k)) (Kn→ΩKn+1 (suc (suc k)) p)) ≡
--                    (cong (cong (minner (suc n) (suc m))) (Kn→ΩKn+10ₖ (3 + k))
--                  ∙∙ sym (Kn→ΩKn+10ₖ (3 + k))
--                  ∙∙ cong (Kn→ΩKn+1 (3 + k)) (Kn→ΩKn+1 (2 + k) (minner (suc n) (suc m) p))
--                  ∙∙ Kn→ΩKn+10ₖ (3 + k)
--                  ∙∙ sym (cong (cong (minner (suc n) (suc m))) (Kn→ΩKn+10ₖ (3 + k))))
--   congcongMinner = {!!}

--   lem₁ : (n m : ℕ) → (a : _) (b : _) →
--          cong (cong (minner (suc n) (suc m)))
--               (cong (Kn→ΩKn+1 (suc (m + suc (suc n)))) (Kn→ΩKn+1 (m + suc (suc n)) (subst coHomK (sym (+-suc m (suc n))) (∪-help m n b a)))) ≡
--          {!!}
--   lem₁ n m a b = rUnit _ ∙∙ {!b!} ∙∙ {!!} ∙∙ {!!} ∙∙ {!!}
