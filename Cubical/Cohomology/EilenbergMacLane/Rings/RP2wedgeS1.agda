...
α² : (x : RP²∨S¹) → _⌣ₖ_ {G'' = ℤ/2Ring} {n = 1} {m = 1} (α-raw x) (α-raw x)
                  ≡ genH²-raw x
α² x = cong (EMTensorMult 2) (α²-raw x)
     ∙ lem x
     ∙ genH²-raw≡ x
  where
  genH²-raw' : RP²∨S¹ → EM ℤ/2 2
  genH²-raw' (inl point) = 0ₖ 2
  genH²-raw' (inl (line i)) = 0ₖ 2
  genH²-raw' (inl (square i j)) = ℤ/2→Ω²K₂' 1 i j
  genH²-raw' (inr x) = 0ₖ 2
  genH²-raw' (push a i) = 0ₖ 2

  genH²-raw≡ : (x : _) → genH²-raw' x ≡ genH²-raw x
  genH²-raw≡ (inl point) = refl
  genH²-raw≡ (inl (line i)) = refl
  genH²-raw≡ (inl (square i j)) k = ℤ/2→Ω²K₂'≡ (~ k) i j
  genH²-raw≡ (inr x) = refl
  genH²-raw≡ (push a i) = refl

  lem : (x : _) → EMTensorMult {G'' = ℤ/2Ring} 2 (genH²-raw⨂ x) ≡ genH²-raw' x
  lem (inl point) = refl
  lem (inl (line i)) = refl
  lem (inl (square i j)) k =
    hcomp (λ r → λ {(i = i0) → EMTensorMult {G'' = ℤ/2Ring} 2 (EM→ΩEM+1-0ₖ 1 (r ∨ k) j)
                   ; (i = i1) → EMTensorMult {G'' = ℤ/2Ring} 2 (EM→ΩEM+1-0ₖ 1 (r ∨ k) j)
                   ; (j = i0) → 0ₖ 2
                   ; (j = i1) → 0ₖ 2
                   ; (k = i0) → EMTensorMult {G'' = ℤ/2Ring} 2
                                  (doubleCompPath-filler (sym (EM→ΩEM+1-0ₖ 1))
                                    (cong (EM→ΩEM+1 1) (EM→ΩEM+1 0 (1 ⊗ 1))) (EM→ΩEM+1-0ₖ 1) r i j)
                   ; (k = i1) → ℤ/2→Ω²K₂' 1 i j})
      (hcomp (λ r → λ {(i = i0) → EMTensorMult {G'' = ℤ/2Ring} 2 ∣ rCancel-filler (merid embase) r k j ∣ₕ
                      ; (i = i1) → EMTensorMult {G'' = ℤ/2Ring} 2 ∣ rCancel-filler (merid embase) r k j ∣ₕ
                      ; (j = i0) → 0ₖ 2
                      ; (j = i1) → EMTensorMult {G'' = ℤ/2Ring} 2 ∣ merid embase (~ r ∧ ~ k) ∣ₕ
                      ; (k = i0) → EMTensorMult {G'' = ℤ/2Ring} 2 ∣ compPath-filler (merid (EM→ΩEM+1 0 (1 ⊗ 1) i)) (sym (merid embase)) r j ∣ₕ
                      ; (k = i1) → ℤ/2→Ω²K₂' 1 i j})
        (hcomp (λ r → λ {(i = i0) → ∣ rCancel-filler (merid embase) (~ r) k j ∣ₕ
                      ; (i = i1) → ∣ rCancel-filler (merid embase) (~ r) k j ∣ₕ
                      ; (j = i0) → 0ₖ 2
                      ; (j = i1) → ∣ merid embase (~ k ∧ r) ∣ₕ -- ∣ merid embase (~ k) ∣ₕ
                      ; (k = i0) → ∣ compPath-filler (merid (EM→ΩEM+1 0 1 i)) (sym (merid embase)) (~ r) j ∣ₕ
                      ; (k = i1) → ℤ/2→Ω²K₂' 1 i j})
...
module _ where
  ...
  ϕ₁≡₄ = (cong₂ _⌣ℤ/2_ ϕ₁Idα ϕ₁Idα
        ∙ cong ∣_∣₂ (funExt α²))
    ∙ sym (retEq (fst H²[RP²∨S¹,ℤ/2]≅ℤ/2) genH²)
    ∙ cong (invEq (fst H²[RP²∨S¹,ℤ/2]≅ℤ/2)) genH²↦1
