...
module Nth-Pol-CommRing (A' : CommRing l) (n : ℕ) where
...
-----------------------------------------------------------------------------
  ...
  Poly-com-adv : (P Q R S : Poly n) →  ((P Poly+ Q) Poly+ (R Poly+ S) ≡ (P Poly+ R) Poly+ (Q Poly+ S))
  ...
  Poly+-Lid : (P : Poly n) → 0P Poly+ P ≡ P
  Poly+-Lid P = (Poly+-comm 0P P) ∙ (Poly+-Rid P)


  Poly-inv : Poly n → Poly n
  Poly-inv = Poly-Rec-Set.f n (Poly n) trunc
             0P
             (λ v a → base v (- a))
             (λ PS RS → PS Poly+ RS)
             Poly+-assoc
  ...
  Poly-invinv : (P : Poly n) → Poly-inv (Poly-inv P) ≡ P
  Poly-invinv = Poly-Ind-Prop.f n (λ P → Poly-inv (Poly-inv P) ≡ P) (λ _ → trunc _ _)
                refl
                (λ v a → cong (base v) (-Idempotent a))
                λ {P Q} ind-P ind-Q → cong₂ _Poly+_ ind-P ind-Q


  Poly+-rinv : (P : Poly n ) → P Poly+ (Poly-inv P) ≡ 0P
  Poly+-rinv = Poly-Ind-Prop.f n (λ P → (P Poly+ Poly-inv P) ≡ 0P) (λ _ → trunc _ _)
               (Poly+-Rid 0P)
  ...
  Poly+-linv : (P  : Poly n) → (Poly-inv P) Poly+ P ≡ 0P
  Poly+-linv = Poly-Ind-Prop.f n _ (λ _ → trunc _ _)
               (Poly+-Rid 0P)
...
-----------------------------------------------------------------------------
  ...
  _Poly*_ = -- Induction Left Argument
            ...
            (λ v    → funExt (
                             ...
                             Poly-Ind-Prop.f n _ (λ _ → trunc _ _)
                             refl
  ...
  0PLeftAnnihilatesPoly : (P : Poly n) → 0P Poly* P ≡ 0P
  0PLeftAnnihilatesPoly P = refl


  0PRightAnnihilatesPoly : (P : Poly n) → P Poly* 0P ≡ 0P
  0PRightAnnihilatesPoly = Poly-Ind-Prop.f n (λ P → (P Poly* 0P) ≡ 0P) (λ _ → trunc _ _)
                           refl
                           (λ _ _ → refl)
                           λ {P Q} ind-P ind-Q → (cong₂ _Poly+_ ind-P ind-Q) ∙ (Poly+-Rid 0P)
  ...
  Poly*-Rid : (P : Poly n) → P Poly* 1P ≡ P
  Poly*-Rid = Poly-Ind-Prop.f n (λ P → (P Poly* 1P) ≡ P) (λ _ → trunc _ _)
              refl
              (λ v a → cong₂ base (+n-vec-rid v) (·Rid a))
              (λ {P Q} ind-P ind-Q → cong₂ _Poly+_ ind-P ind-Q)


  Poly*-Lid : (P : Poly n) → 1P Poly* P ≡ P
  Poly*-Lid = Poly-Ind-Prop.f n (λ P → (1P Poly* P) ≡ P) (λ _ → trunc _ _)
              refl
              (λ v a → cong₂ base (+n-vec-lid v) (·Lid a))
              λ {P Q} ind-P ind-Q → cong₂ _Poly+_ ind-P ind-Q


  Poly*-Rdist : (P Q R : Poly n) → P Poly* (Q Poly+ R) ≡ (P Poly* Q) Poly+ (P Poly* R)
  Poly*-Rdist = Poly-Ind-Prop.f n _
  ...
  Poly*-Ldist : (P Q R : Poly n) → (P Poly+ Q) Poly* R ≡ (P Poly* R) Poly+ (Q Poly* R)
  Poly*-Ldist P Q R = refl


  Poly*-comm : (P Q : Poly n) → P Poly* Q ≡ Q Poly* P
  Poly*-comm = Poly-Ind-Prop.f n _
...
-----------------------------------------------------------------------------
  ...
  nth-Pol-CommRing : CommRing l
  fst nth-Pol-CommRing = Poly n
  0r (snd nth-Pol-CommRing) = 0P
  1r (snd nth-Pol-CommRing) = 1P
  _+_ (snd nth-Pol-CommRing) = _Poly+_
  _·_ (snd nth-Pol-CommRing) = _Poly*_
  - snd nth-Pol-CommRing = Poly-inv
  is-set (isSemigroup (isMonoid (isGroup (IsRing.+IsAbGroup (IsCommRing.isRing (isCommRing (snd nth-Pol-CommRing))))))) = trunc
  IsSemigroup.assoc (isSemigroup (isMonoid (isGroup (IsRing.+IsAbGroup (IsCommRing.isRing (isCommRing (snd nth-Pol-CommRing))))))) = Poly+-assoc
  identity (isMonoid (isGroup (IsRing.+IsAbGroup (IsCommRing.isRing (isCommRing (snd nth-Pol-CommRing)))))) P = (Poly+-Rid P) , (Poly+-Lid P)
  inverse (isGroup (IsRing.+IsAbGroup (IsCommRing.isRing (isCommRing (snd nth-Pol-CommRing))))) = λ P → Poly+-rinv P , Poly+-linv P
  comm (IsRing.+IsAbGroup (IsCommRing.isRing (isCommRing (snd nth-Pol-CommRing)))) = Poly+-comm
  is-set (isSemigroup (IsRing.·IsMonoid (IsCommRing.isRing (isCommRing (snd nth-Pol-CommRing))))) = trunc
  IsSemigroup.assoc (isSemigroup (IsRing.·IsMonoid (IsCommRing.isRing (isCommRing (snd nth-Pol-CommRing))))) = Poly*-assoc
  identity (IsRing.·IsMonoid (IsCommRing.isRing (isCommRing (snd nth-Pol-CommRing)))) P = (Poly*-Rid P) , (Poly*-Lid P)
  IsRing.dist (IsCommRing.isRing (isCommRing (snd nth-Pol-CommRing))) P Q R = Poly*-Rdist P Q R , Poly*-Ldist P Q R
  IsCommRing.·Comm (isCommRing (snd nth-Pol-CommRing)) = Poly*-comm
