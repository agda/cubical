...
module BooleanAlgebraStr (A : BooleanRing ℓ)  where
  ...
  ∨Comm : (x ∨ y ) ≡ (y ∨ x)
  ∨Comm  = solve! (BooleanRingToCommRing A)
  ...
  -IsId : x + x ≡ 𝟘
  -IsId {x = x} =  RingTheory.+Idempotency→0 (CommRing→Ring (BooleanRingToCommRing A)) (x + x) 2x≡4x
    where
      2x≡4x : x + x ≡ (x + x) + (x + x)
      2x≡4x =
        (x + x)
  ...
  DeMorgan¬∨ : ¬ (x ∨ y) ≡ (¬ x) ∧ (¬ y)
  DeMorgan¬∨ = solve! (BooleanRingToCommRing A)
