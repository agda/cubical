...
ℤ→Fin n (pos x) = x mod (suc n) , <→<ᵗ (mod< n x)
ℤ→Fin n (negsuc x) = -ₘ (suc x mod suc n , <→<ᵗ (mod< n (suc x)))

ℤ→Fin-presinv : (n : ℕ) (x : ℤ) → ℤ→Fin n (- x) ≡ -ₘ ℤ→Fin n x
ℤ→Fin-presinv n (pos zero) =
  Σ≡Prop (λ z → isProp<ᵗ {n = z} {suc n}) ((λ _ → zero) ∙ sym (cong fst help))
  where
  help : (-ₘ_ {n = n} 0) ≡ 0
  help = GroupTheory.inv1g (ℤGroup/ (suc n))
ℤ→Fin-presinv n (pos (suc x)) = Σ≡Prop (λ z → isProp<ᵗ {n = z} {suc n}) refl
...
suc-ₘ1 : (n y : ℕ)
  → ((suc y mod suc n) , <→<ᵗ (mod< n (suc y))) -ₘ (1 mod (suc n) , <→<ᵗ (mod< n 1))
   ≡ (y mod suc n , <→<ᵗ (mod< n y))
suc-ₘ1 zero y =
  isContr→isProp
    (isOfHLevelRetractFromIso 0 (fst ℤGroup/1≅Unit) isContrUnit) _ _
suc-ₘ1 (suc n) y =
     (λ i → ((suc y mod suc (suc n)) , <→<ᵗ (mod< (suc n) (suc y)))
         +ₘ (-ₘ1-id (suc n) i))
   ∙ Σ≡Prop (λ z → isProp<ᵗ {n = z} {suc (suc n)})
      (cong (_mod (2 +ℕ n))
...
1-ₘsuc : (n y : ℕ)
  →     ((1 mod (suc n) , <→<ᵗ (mod< n 1))
     +ₘ (-ₘ (((suc y mod suc n) , <→<ᵗ (mod< n (suc y))))))
       ≡ -ₘ ((y mod suc n) , <→<ᵗ (mod< n y))
1-ₘsuc n y =
     sym (GroupTheory.invInv (ℤGroup/ (suc n)) _)
   ∙ cong -ₘ_
...
isHomℤ→Fin n =
  ...
  +1case (negsuc (suc y)) =
    Σ≡Prop (λ z → isProp<ᵗ {n = z} {suc n})
      ...
      ∙∙ cong fst (cong -ₘ_ (refl {x = suc y mod suc n , <→<ᵗ (mod< n (suc y))}))
      ∙∙ cong fst (sym (1-ₘsuc n (suc y)))
       ∙ λ i → fst ((1 mod (suc n) , <→<ᵗ (mod< n 1))
         +ₘ (-ₘ (((suc (suc y) mod suc n) , <→<ᵗ (mod< n (suc (suc y))))))))
...
ℤ/2-elim {A = A} a₀ a₁ (zero , p) = a₀
ℤ/2-elim {A = A} a₀ a₁ (suc zero , p) = a₁
ℤ/2-elim {A = A} a₀ a₁ (suc (suc x) , p) = ⊥.rec p

ℤ/2-rec : ∀ {ℓ} {A : Type ℓ} → A → A → Fin 2 → A
ℤ/2-rec {A = A} a₀ a₁ (zero , p) = a₀
ℤ/2-rec {A = A} a₀ a₁ (suc zero , p) = a₁
ℤ/2-rec {A = A} a₀ a₁ (suc (suc x) , p) = ⊥.rec p
