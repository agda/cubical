...
isSetGroupHom {G = G} {H = H} = isOfHLevelRespectEquiv 2 equiv (isSetΣ (isSetΠ λ _ → is-set (snd H)) λ _ → isProp→isSet (isPropIsGroupHom G H)) where
  ...
  equiv =  isoToEquiv (λ (g , m) → grouphom g m) (λ hom → fun hom , isHom hom) (λ _ → refl) λ _ → refl
...
Group≡ G H = isoToEquiv
  (λ{ (p , q , r , s , t) i → p i , groupstr (q i) (r i) (s i) (t i) })
  (λ x → cong ⟨_⟩ x , cong (0g ∘ snd) x , cong (_+_ ∘ snd) x , cong (-_ ∘ snd) x , cong (isGroup ∘ snd) x)
  (λ _ → refl)
  (λ _ → refl)
