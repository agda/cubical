  ...
  main zero (suc m) x =
    ...
    help zero =
      wedgeConEM.fun _ _ _ _
        ...
        (λ z → cong (c zero x) (lUnitₖ _ (EM-raw→EM H' 1 z)) ∙ refl)
    ...
    help (suc m) =
      wedgeConEM.fun _ _ _ _
        ...
        refl
  main (suc n) zero =
    EM'-elim G' (suc n) (λ _ → isOfHLevelΠ2 (suc (suc n)) λ _ _ → hLevelEM _ (suc n) _ _)
      (lem n (main n zero))
    where
    cup : (n : ℕ) → _
    ...
    lem : (n : ℕ) (ind : (x : EM G' n) (y z : fst H')
          ...
          (x : EM' G' (suc n)) (y z : fst H')
        → cup (suc n) (EM'→EM G' (suc n) x) (y +ₖ z)
        ≡ cup (suc n) (EM'→EM G' (suc n) x) y +ₖ cup (suc n) (EM'→EM G' (suc n) x) z
...
module RightDistributivity {G' : AbGroup ℓ} {H' : AbGroup ℓ'} where
  ...
  main zero (suc m) x y =
    EM'-elim _ _ (λ _ → hLevelEM _ (suc m) _ _)
      (lem m (main zero m) x y)
    where
    cup : (m : ℕ) (x : G) → _
    cup m g = cup∙ {G' = G'} {H' = H'} zero m g .fst

    lem : (m : ℕ) → ((x y : G) (z : EM H' m) → cup m (x +G y) z ≡ cup m x z +ₖ cup m y z)
      → ((x y : G) (z : EM' H' (suc m))
      → cup (suc m) (x +G y) (EM'→EM _ _ z)
       ≡ cup (suc m) x (EM'→EM _ _ z) +ₖ cup (suc m) y (EM'→EM _ _ z))
    lem zero ind x y embase-raw = refl
    lem zero ind x y (emloop-raw g i) = flipSquare help i
      where
      help : cong (cup 1 (x +G y)) (emloop g)
           ≡ cong₂ _+ₖ_ (cong (cup 1 x) (emloop g)) (cong (cup 1 y) (emloop g))
    ...
    lem (suc m) ind x y (merid a i) = flipSquare help i
      ...
      help = cong (EM→ΩEM+1 (suc m)) (ind x y (EM-raw→EM _ _ a))
          ∙∙ EM→ΩEM+1-hom (suc m) (cup (suc m) x (EM-raw→EM _ _ a)) (cup (suc m) y (EM-raw→EM _ _ a))
          ∙∙ sym (cong₂+₂ _ _ _)
    
  main (suc n) zero =
    EM-elim2 _ (λ _ _ → isOfHLevelΠ (3 + n) λ _ → isOfHLevelPath (3 + n) (hLevelEM _ (suc n)) _ _)
      λ x y z → lem n z x y
    where
    cup : (n : ℕ) (x : EM G' n) → _
    ...
    lem zero z =
      wedgeConEM.fun _ _ _ _
        ...
        (λ x → cong (λ x → cup 1 x z) (lUnitₖ _ (EM-raw→EM G' 1 x)) ∙ refl)
        (λ y → cong (λ x → cup 1 x z) (rUnitₖ _ (EM-raw→EM G' 1 y)) ∙ sym (rUnitₖ _ (cup 1 (EM-raw→EM G' 1 y) z)))
        refl
    lem (suc n) z =
      wedgeConEM.fun _ _ _ _
        (λ _ _ → isOfHLevelPath (2 + n + (2 + n))
                   (subst (λ m → isOfHLevel (suc (suc m)) (EM (G' ⨂ H') (suc (suc n))))
                          (+-comm (suc (suc n)) n)
                          (isOfHLevelPlus' {n = n} (4 + n) (hLevelEM _ (suc (suc n))))) _ _)
        (λ x → cong (λ x → cup (suc (suc n)) x z) (lUnitₖ _ (EM-raw→EM G' _ x))
              ∙ sym (lUnitₖ (suc (suc n)) (cup (suc (suc n)) (EM-raw→EM G' (suc (suc n)) x) z)))
        (λ y → cong (λ x → cup (suc (suc n)) x z) (rUnitₖ _ (EM-raw→EM G' _ y))
              ∙ sym (rUnitₖ (suc (suc n)) (cup (suc (suc n)) (EM-raw→EM G' (suc (suc n)) y) z)))
        refl
