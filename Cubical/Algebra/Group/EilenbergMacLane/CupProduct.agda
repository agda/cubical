...
module Assoc {G' : AbGroup ℓ} {H' : AbGroup ℓ'} {L' : AbGroup ℓ''} where
   private
     G = fst G'
     H = fst H'
     L = fst L'
     ...
     open import Cubical.Data.Sigma
     assocInd : (n m l : ℕ) (f g : (EM∙ G' n →∙ (EM∙ H' m →∙ (EM∙ L' l →∙ EM∙ ((G' ⨂ H') ⨂ L') ((n +' m) +' l) ∙) ∙)))
              → ((x : EM' G' n) (y : EM' H' m) (z : EM' L' l) → fst f (EM'→EM _ n x) .fst (EM'→EM _ m y) .fst (EM'→EM _ l z)
     ...
     assocInd zero (suc m) (suc l) f g ind =
       →∙Homogeneous≡ (isHomogeneous→∙ (isHomogeneous→∙ (isHomogeneousEM _)))
         (funExt λ x → →∙Homogeneous≡ (isHomogeneous→∙ (isHomogeneousEM _))
           (funExt (EM'-elim _ _
             (λ _ → isOfHLevelPath' (suc (suc m))
               (isOfHLevel↑∙ {G = L'} {H = (G' ⨂ H') ⨂ L'} (suc m) l) _ _)
               λ y → →∙Homogeneous≡ (isHomogeneousEM _)
                 (funExt λ z → funExt⁻ (cong fst (helpid x z)) y))))
       where
       help : (EM∙ G' zero →∙ (EM∙ H' (suc m) →∙ EM∙ L' (suc l)
           →∙ EM∙ ((G' ⨂ H') ⨂ L') ((zero +' suc m) +' suc l) ∙ ∙))
         → (x : fst G') → EM L' (suc l) → EM'∙ H' (suc m) →∙ EM∙ ((G' ⨂ H') ⨂ L') ((suc m) +' suc l)
       ...
       helpid x =
         EM'-elim _ _
           (λ x → isOfHLevelPath' (suc (suc l)) {!!} _ _)
     ...
     assocInd (suc n) (suc m) (suc l) f g ind =
       ...
       F₂ : (f : (EM∙ G' (suc n) →∙ (EM∙ H' (suc m) →∙ (EM∙ L' (suc l)
              →∙ EM∙ ((G' ⨂ H') ⨂ L') (((suc n) +' (suc m)) +' (suc l)) ∙) ∙)))
         → EM∙ H' (suc m) →∙ (EM∙ L' (suc l) →∙ EM'∙ G' (suc n)
         →∙ EM∙ ((G' ⨂ H') ⨂ L') (suc (suc (suc (n + m + l)))) ∙ ∙)
       ...
       F₃id = funExt
               (EM'-elim _ _ (λ _ → isOfHLevelPath' (suc (suc l))
                                    (subst2 (λ y x → isOfHLevel y
                                      ((EM'∙ H' (suc m) →∙ (EM'∙ G' (suc n) →∙ EM∙ ((G' ⨂ H') ⨂ L') x ∙))))
                                    refl
                                    (cong (λ x → suc (suc (suc x)))
                                      ...
                                      ∙ +-assoc m n l ∙
                                      cong (_+ l) (+-comm m n)))
                                    (isOfHLevel↑∙∙'' {G = H'} {H = G'} {L = ((G' ⨂ H') ⨂ L') } m n (suc l))) _ _)
                 λ x → →∙Homogeneous≡ (isHomogeneous→∙ (isHomogeneousEM _))
                   (funExt λ y → →∙Homogeneous≡ (isHomogeneousEM _)
                     (funExt (λ z → ind z y x))))

       help : isOfHLevel (suc (2 + m)) (EM∙ L' (suc l) →∙ (EM'∙ G' (suc n) →∙ EM∙ ((G' ⨂ H') ⨂ L') (suc (suc (suc (n + m + l)))) ∙))
       help =
         subst2 (λ x y → isOfHLevel x (EM∙ L' (suc l) →∙ (EM'∙ G' (suc n) →∙ EM∙ ((G' ⨂ H') ⨂ L') y ∙)))
                refl
                (cong (λ x → suc (suc (suc x)))
                  (+-comm (m + l) n ∙ +-assoc n m l))
                (isOfHLevel↑∙∙' {G = L'} {H = G'} {L = ((G' ⨂ H') ⨂ L') } l n (suc m))
