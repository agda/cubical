...
univP : ∀ {ℓ} (A B : Type ℓ) → Type _
univP {ℓ = ℓ} A B = (C : Type ℓ) → Iso (A → C) (B → C)

univP' : ∀ {ℓ} (A B : Type ℓ) → Type _
univP' {ℓ = ℓ} A B = Σ[ f ∈ ((C : Type ℓ) → Iso (A → C) (B → C)) ] Iso.fun (f B) ≡ λ _ b → b

univP'' : ∀ {ℓ} (A B : Type ℓ) → Type _
univP'' {ℓ = ℓ} A B = (P : (C : Type ℓ) → Type ℓ) → Iso ((x : A) → (P A)) ((x : B) → (P B))

univP→Id :  ∀ {ℓ} (A B : Pointed ℓ) → univP'' (typ A) (typ B) → Iso (typ A) (typ B)
univP→Id A B is = Iso.inv (is (λ C → Iso C (typ B))) (λ _ → idIso) (pt A)

open import Cubical.HITs.SmashProduct

test : ∀ {ℓ} (A B C : Pointed ℓ) → univP'' (Smash (SmashPt A B) C) (Smash A (SmashPt B C))
test A B C P = {!!}
  where
  h' : (Smash (SmashPt A B) C → P (Smash (SmashPt A B) C))
  h' basel = {!!}
  h' baser = {!!}
  h' (proj x y) = {!!}
  h' (gluel a i) = {!!}
  ...
  h : (Smash (SmashPt A B) C → P (Smash (SmashPt A B) C)) → Σ[ l ∈ P (Smash (SmashPt A B) C) ] {!!}
  h = {!!}
...
-- Iso→EMIso : {G : AbGroup ℓ} {H : AbGroup ℓ'} → AbGroupEquiv G H → ∀ n → Iso (EM G n) (EM H n) 
...
-- EM⊗-commFun : {G : AbGroup ℓ} {H : AbGroup ℓ'} → ∀ n →  Iso (EM (G ⨂ H) n) (EM (H ⨂ G) n)
-- EM⊗-commFun {G = G} {H = H} = Iso→EMIso (GroupIso→GroupEquiv ⨂-commIso)

-- EM⊗-assocIso : {G : AbGroup ℓ} {H : AbGroup ℓ'} {L : AbGroup ℓ''} → ∀ n → Iso (EM (G ⨂ (H ⨂ L)) n) (EM ((G ⨂ H) ⨂ L) n)
-- EM⊗-assocIso = Iso→EMIso (GroupIso→GroupEquiv (GroupEquiv→GroupIso ⨂assoc))

-- EM⊗-raw-comm-sect : {G : AbGroup ℓ} {H : AbGroup ℓ'} {L : AbGroup ℓ''} → ∀ n → Iso (EM (G ⨂ H) n) (EM (H ⨂ G) n)
-- EM⊗-raw-comm-sect = {!!}
