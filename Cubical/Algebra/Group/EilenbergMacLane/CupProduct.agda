...
stupidInd {A = A} a0 a1 ind (suc (suc n)) = ind n (stupidInd {A = A} a0 a1 ind (suc n))
...
inducedFun-EM-raw-sect : {G' : AbGroup ℓ} {H' : AbGroup ℓ'}
                     → (f : AbGroupEquiv G' H')
                     → ∀ n
                     →  (x : EM-raw G' n) → inducedFun-EM-raw (invEq (fst f) , isGroupHomInv f) n
                                               (inducedFun-EM-raw (fst (fst f) , snd f) n x)
                                             ≡ x
inducedFun-EM-raw-sect f =
  stupidInd (λ x → {!!})
            {!!}
            {!!}
...
inducedFun-EM-rawIso : {G' : AbGroup ℓ} {H' : AbGroup ℓ'}
                     → AbGroupEquiv G' H'
                     → ∀ n
                     →  Iso (EM-raw G' n) (EM-raw H' n)
inducedFun-EM-rawIso e n = {!!}

EM⊗-raw-commFun : {G' : AbGroup ℓ} {H' : AbGroup ℓ'} → ∀ n →  EM-raw (G' ⨂ H') n → EM-raw (H' ⨂ G') n
EM⊗-raw-commFun =
  stupidInd commFun
    (EMrec _ emsquash embase
     (λ g → emloop (commFun g))
      λ g h → compPathR→PathP (sym (sym (lUnit _)
                             ∙∙ cong (_∙ sym (emloop (commFun h))) (emloop-comp _ (commFun g) (commFun h))
                             ∙∙ sym (∙assoc _ _ _)
                             ∙∙ cong (emloop (commFun g) ∙_) (rCancel _)
                             ∙∙ sym (rUnit _))))
    (λ n ind → λ { north → north
                  ; south → south
                  ; (merid a i) → merid (ind a) i} )
...
module _ {G' : AbGroup ℓ} {H' : AbGroup ℓ'} where
   ...
   precup : ∀ n m → EM-raw G' n → EM∙ H' m →∙ EM∙ (G' ⨂ H') (n +' m)
   precup zero m = {!!}
   precup (suc zero) m = {!!}
   precup (suc (suc n)) m = {!!}
   ...
   private
     distrr1 : (n m : ℕ) → EM H' n → EM H' n → EM∙ G' m →∙ EM∙ (H' ⨂ G') (n +' m)
     fst (distrr1 n m x y) z = {!!} -- (x +ₖ y) ⌣ₖ ?
     snd (distrr1 n m x y) = {!!} -- ⌣ₖ-0ₖ n m _

     distrr2 : (n m : ℕ) → EM H' m → EM H' m → EM∙ G' n →∙ EM∙ (G' ⨂ H') (n +' m)
     fst (distrr2 n m x y) z = (z ⌣ₖ x) +ₖ (z ⌣ₖ y)
     snd (distrr2 n m x y) = cong₂ _+ₖ_ (0ₖ-⌣ₖ n m x) (0ₖ-⌣ₖ n m y) ∙ rUnitₖ _ (0ₖ (n +' m))
   ...
   mainDistrR : {!(n m : ℕ) (x y : EM H' (suc m)) → distr1 (suc n) (suc m) x y ≡ distrl2 (suc n) (suc m) x y!}
   mainDistrR = {!!}
