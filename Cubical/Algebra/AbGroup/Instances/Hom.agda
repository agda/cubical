...
module _ (A : AbGroup ℓ) (B : AbGroup ℓ') where
  ...
  private
    ...
    hom0AB f = hom1g (AbGroupStr→GroupStr (A .snd)) (f .fst) (AbGroupStr→GroupStr (B .snd)) (f .snd .pres·)

    homInvAB : (f : AbGroupHom A B) → (a : A .fst) → f .fst (inv a) ≡ (- f .fst a)
    homInvAB f a = homInv (AbGroupStr→GroupStr (A .snd)) (f .fst) (AbGroupStr→GroupStr (B .snd)) (f .snd .pres·) a
...
HomIsAb : (A : AbGroup ℓ) → (B : AbGroup ℓ') → AbGroupStr (AbGroupHom A B)
HomIsAb A B = abgroupstr
  (zero A B)
  (HomAdd A B)
  (HomInv A B)
  (makeIsAbGroup
