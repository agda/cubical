...
module Determinat (ℓ : Level) (P' : CommRing ℓ) where
  ...
  opaque
    ...
    MFplusZero : (i : Fin n) →   MF (toℕ i +ℕ zero) ≡ MF (toℕ i)
    MFplusZero i =
      MF (toℕ i +ℕ zero)
      ≡⟨ sumMF (toℕ i) zero ⟩
      (MF (toℕ i) · MF zero)
  ...
  opaque
    detComp : (M N : FinMatrix R n n) → ((i j : Fin n) → M i j ≡ N i j ) → det M ≡ det N
    detComp M N p =
      let q : M ≡ N
          q k i j = p i j k
      in cong det q
  ...
  opaque
    ...
    DetRowAux4a : (k : Fin (suc n)) → (M : FinMatrix R (suc (suc n)) (suc (suc n))) →
      ∑ (λ i →
           ∑ (λ j →
             ind> (toℕ (suc i)) (toℕ j) ·
  ...
  opaque
    DetRowAux4b : (k : Fin (suc n)) → (M : FinMatrix R (suc (suc n)) (suc (suc n))) →
      ∑
       (λ i →
          ∑
          (λ j →
    ...
    DetRowAux5a : (k : Fin (suc n)) → (M : FinMatrix R (suc (suc n)) (suc (suc n))) →
      ∑
       (λ i →
          ∑
          (λ j →
    ...
    DetRowAux6a : (k : Fin (suc n)) → (M : FinMatrix R (suc (suc n)) (suc (suc n))) →
      ∑
        (λ j →
           ∑
           (λ i →
    ...
    DetRowAux6bAux : (k : Fin (suc n)) → (i j : Fin (suc n)) →
      MF (toℕ (suc k) +ℕ toℕ (weakenFin i) +ℕ toℕ j)
      ≡
    ...
    DetRowAux6b : (k : Fin (suc n)) → (M : FinMatrix R (suc (suc n)) (suc (suc n))) →
       ∑
       (λ j →
          ∑
          (λ i →
  ...
  opaque
    ...
    DetRowAux7a : (k : Fin (suc n)) → (M : FinMatrix R (suc (suc n)) (suc (suc n))) →
      ∑
       (λ j →
          ∑
          (λ i →
    ...
    DetRowAux7b : (k : Fin (suc n)) → (M : FinMatrix R (suc (suc n)) (suc (suc n))) →
      ∑
         (λ j →
            ∑
            (λ i →
    ...
    DetRowAux8 : (i : Fin (suc (suc n))) → (j k : Fin (suc n))
       → (M : FinMatrix R (suc (suc n)) (suc (suc n))) →
  ...
  opaque
    DetRowColumnAux : (M : FinMatrix R (suc (suc n)) (suc (suc n))) →
      ∑
        (λ i →
           ∑
           (λ j →
    ...
    DetColumnAux1a : (k : Fin (suc n)) → (M : FinMatrix R (suc (suc n)) (suc (suc n))) →
      ∑
        (λ i →
           ∑
           (λ j →
      ...
      ∑∑Compat
        ...
        (λ i j →
         ...
         ≡⟨
           ...
           ≡⟨ sumMF (toℕ j) zero ⟩
           (MF (toℕ j) · MF zero)
           ≡⟨ cong (λ a → MF a · MF zero) ((toℕ j) ≡⟨ sym (weakenRespToℕ j) ⟩ toℕ (weakenFin j) ∎) ⟩
         ...
         ≡⟨
           ...
           (MF (toℕ (suc i) +ℕ toℕ (suc k)) · M (suc i) (suc k) ·
            (MF (toℕ (weakenFin j) +ℕ zero) · M (weakenFin j) zero ·
             det (minor i k (minor (weakenFin j) zero M)))))
           (toℕ j ≡⟨ sym (weakenRespToℕ j) ⟩ toℕ (weakenFin j) ∎)
          ⟩
         ((1r + - ind> (toℕ (weakenFin j)) (toℕ i)) ·
           (MF (toℕ (suc i) +ℕ toℕ (suc k)) · M (suc i) (suc k) ·
            (MF (toℕ (weakenFin j) +ℕ zero) · M (weakenFin j) zero ·
    ...
    DetColumnAux1b : (k : Fin (suc n)) → (M : FinMatrix R (suc (suc n)) (suc (suc n))) →
      ∑
        (λ i →
           ∑
           (λ j →
    ...
    DetColumnAux2a : (k : Fin (suc n)) → (M : FinMatrix R (suc (suc n)) (suc (suc n))) →
      ∑
        (λ i →
           ∑
           (λ j →
    ...
    DetColumnAux2b : (k : Fin (suc n)) → (M : FinMatrix R (suc (suc n)) (suc (suc n))) →
      ∑
        (λ i →
           ∑
           (λ j →
    ...
    DetColumn : (k : Fin (suc n)) → (M : FinMatrix R (suc n) (suc n)) →
      detC k M ≡ det M
    DetColumn k M =
     detC k M
     ≡⟨ DetColumnZero k M ⟩
