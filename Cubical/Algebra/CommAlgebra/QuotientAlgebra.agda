...
module _ {R : CommRing {ℓ}} (A : CommAlgebra R) where
  open CommRingStr {{...}} hiding (_-_; -_; dist; ·Lid) renaming (_·_ to _·R_; _+_ to _+R_)
  open CommAlgebra {{...}}
  open Theory (CommRing→Ring (CommAlgebra→CommRing A)) using (-DistR·)
  instance
    _ : CommRingStr _
    _ = snd R
    _ : CommAlgebra R
  ...
  _/_ : (I : IdealsIn A) → CommAlgebra R
  _/_ I = commAlgebraFromCommRing
          ...
          where
                eq : (r : fst R) (x y : Carrier) → x - y ∈ (fst I) →  [ r ⋆ x ] ≡ [ r ⋆ y ]
                eq r x y x-y∈I = eq/ _ _
                  (subst (λ u → u ∈ fst I)
                  ((r ⋆ 1a) · (x - y)               ≡⟨ fst (dist (r ⋆ 1a) x (- y)) ⟩
...
[_]/ : {R : CommRing {ℓ}} {A : CommAlgebra R} {I : IdealsIn A}
       → (a : CommAlgebra.Carrier A) → CommAlgebra.Carrier (A / I)
[ a ]/ = [ a ]
