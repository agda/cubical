...
module Minor (ℓ : Level) where
  ...
  removeIndexId : {n : ℕ} → (i : Fin (suc n)) → (k : Fin n) → (suc k) ≤'Fin i →
    (removeIndex i k) ≡ weakenFin k
  ...
  removeIndexSuc : {n : ℕ} → (i : Fin (suc n)) → (k : Fin n) → i ≤'Fin weakenFin k  →
    (removeIndex i k) ≡ suc k
  ...
  removeCol : {A : Type ℓ} {n m : ℕ} (j : Fin (suc m)) (M : FinMatrix A n (suc m)) → FinMatrix A n m
  removeCol j M k l = M k (removeIndex j l)

  removeCol-comm :  {A : Type ℓ} {n m : ℕ} →
    (i : Fin (suc m)) (j : Fin (suc m)) →
    (M : FinMatrix A n (suc (suc m))) →
    (k : Fin n) (l : Fin  m) →
    j ≤'Fin i →
    removeCol j (removeCol (suc i) M) k l ≡  removeCol i (removeCol (weakenFin j) M) k l
  removeCol-comm i j M k l le =
    removeCol j (removeCol (suc i) M) k l
    ≡⟨  refl ⟩
     M k (removeIndex (suc i) (removeIndex j l))
    ≡⟨ cong (λ a → M k a) ( removeIndexComm j i l le) ⟩
    M k (removeIndex (weakenFin j) (removeIndex i l))
    ≡⟨ refl ⟩
     removeCol i (removeCol (weakenFin j) M) k l
    ∎

  -- remove the i-th row of a matrix
  removeRow : {A : Type ℓ} {n m : ℕ} →
    (i : Fin (suc n)) (M : FinMatrix A (suc n) m) →
    FinMatrix A n m
  removeRow i M k l = M (removeIndex i k) l

  removeRow-comm :
    {A : Type ℓ} {n m : ℕ} →
    (i : Fin (suc n)) (j : Fin (suc n)) →
    (M : FinMatrix A (suc (suc n)) m) →
    (k : Fin n) (l : Fin  m) →
    j ≤'Fin i →
    removeRow j (removeRow (suc i) M) k l ≡  removeRow i (removeRow (weakenFin j) M) k l
  removeRow-comm i j M k l le =
    removeRow j (removeRow (suc i) M) k l
    ≡⟨ refl ⟩
      M (removeIndex (suc i) (removeIndex j k)) l
    ≡⟨ cong (λ a → M a l) ( removeIndexComm j i k le) ⟩
      M (removeIndex (weakenFin j) (removeIndex i k)) l
    ≡⟨ refl ⟩
     removeRow i (removeRow (weakenFin j) M) k l
    ∎

  removeColRow-comm :
    {A : Type ℓ} {n m : ℕ} →
    (i : Fin (suc n)) (j : Fin (suc m)) →
    (M : FinMatrix A (suc n) (suc m)) →
    (k : Fin n) (l : Fin m) →
    (removeRow i (removeCol j M)) k l
      ≡ (removeCol j (removeRow i M)) k l
  removeColRow-comm i j M k l = refl

  -- Calculating of the minor.
  minor : {A : Type ℓ} {n m : ℕ} →
    (i : Fin (suc n)) (j : Fin (suc m)) →
    (M : FinMatrix A (suc n) (suc m)) →
    FinMatrix A n m
  minor i j M = removeCol j (removeRow i M)

  -- Compatability of the minor with the equality.
  minorComp : {A : Type ℓ} {n m : ℕ} →
    (i : Fin (suc n)) (j : Fin (suc m))→
    (M N : FinMatrix A (suc n) (suc m)) →
    ((i : Fin (suc n)) → (j : Fin (suc m)) → M i j ≡ N i j ) →
    (k : Fin n) →
    (l : Fin m) →  minor i j M k l ≡ minor i j N k l
  minorComp {n = suc n} {suc m} i j M N f k l = f (removeIndex i k) (removeIndex j l)

  -- Formulas to commute minors
  minorComm0 : {A : Type ℓ} {n m : ℕ}
    (i₁ i₂ : Fin (suc n)) (j₁ j₂ : Fin (suc m))
    (k : Fin n) (l : Fin m)
    (M : FinMatrix A (suc (suc n)) (suc (suc m))) →
    i₂ ≤'Fin i₁ → j₂ ≤'Fin j₁ →
  ...
  minorSemiCommR : {A : Type ℓ} {n m : ℕ} →
    (i₁ : Fin (suc (suc n))) (i₂ : Fin (suc n)) (j₁ j₂ : Fin (suc m)) →
    (k : Fin n) (l : Fin m) →
    (M : FinMatrix A (suc (suc n)) (suc (suc m))) →
    j₂ ≤'Fin j₁ →
  ...
  minorIdId : {A : Type ℓ} {n m : ℕ} →
    (i : Fin (suc n))(j : Fin (suc m)) (k : Fin n)(l : Fin m) →
    (M : FinMatrix A (suc n) (suc m)) →
