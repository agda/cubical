...
shift-iso {S = S@(A , B)} = compIso sym-α-iso-step-6 (compIso (isoInv α-iso-step-5-Iso) α-iso-step-1-4-Iso-lem-12)
  where
    α-iso-step-1-4-Iso-lem-12 : Iso (Σ (Σ ((n : ℕ) → A) (λ a → (n : ℕ) → a (suc n) ≡ a n)) λ a →
    ...
    fun (lemma10-Iso) f = (λ n z → (f z) .fst n) , λ n i a → (f a) .snd n i
