...
rightist-group : ∀ {ℓ} {A : Type ℓ}
  ...
  → Group
rightist-group {A = A} comp set assoc id rUnit inv rCancel =
  A , comp , (set , assoc) , id , (λ x → rUnit x , lUnit x) , λ x → inv x , rCancel x , lCancel x
  where
    _⨀_ = comp
    abstract
...
leftist-group : ∀ {ℓ} {A : Type ℓ}
  ...
  → (lCancel : ∀ a → comp (inv a) a ≡ id)
  → Group
leftist-group {A = A} comp set assoc id lUnit inv lCancel =
  A , comp , (set , assoc) , id , (λ x → rUnit x , lUnit x) , λ x → inv x , rCancel x , lCancel x
  where
    abstract
      rCancel : ∀ a → comp a (inv a) ≡ id
