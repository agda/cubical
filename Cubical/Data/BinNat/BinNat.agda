...
Binℕ≃ℕ : Binℕ ≃ ℕ
Binℕ≃ℕ = isoToEquiv (Binℕ→ℕ , record { inverse = ℕ→Binℕ ; rightInv = ℕ→Binℕ→ℕ ; leftInv = Binℕ→ℕ→Binℕ})
...
ℕ≃binnat : ℕ ≃ binnat
ℕ≃binnat = isoToEquiv (ℕ→binnat , record { inverse = binnat→ℕ ; rightInv = binnat→ℕ→binnat ; leftInv = ℕ→binnat→ℕ })
