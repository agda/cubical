...
private opaque
  ...
  _≤'_ = rec2 isSetHProp
    ...
    (λ a/b c/d e/f ad≡cb → Σ≡Prop (λ _ → isPropIsProp) (lemma≤₁ a/b c/d e/f ad≡cb))
     λ a/b c/d e/f cf≡ed → Σ≡Prop (λ _ → isPropIsProp) (lemma≤₂ a/b c/d e/f cf≡ed)
    where
      abstract
        lemma≤₁ : ((a , b) (c , d) (e , f) : (ℤ × ℕ₊₁))
                → (a ℤ.· ℕ₊₁→ℤ d ) ≡ (c ℤ.· ℕ₊₁→ℤ b)
                → ((a ℤ.· ℕ₊₁→ℤ f) ℤ.≤ (e ℤ.· ℕ₊₁→ℤ b))
                ≡ ((c ℤ.· ℕ₊₁→ℤ f) ℤ.≤ (e ℤ.· ℕ₊₁→ℤ d))
        lemma≤₁ (a , b) (c , d) (e , f) ad≡cb = (ua (propBiimpl→Equiv ℤ.isProp≤ ℤ.isProp≤
                (ℤ.≤-·o-cancel {k = -1+ b} ∘
...
module _ where opaque
  ...
  {- Since the internal definitions are private and opaque
     (and hence inaccessible outside of this file),
     these functions allow for using ≤ and < for direct
     examples that need to convert to/from the integer form.
  -}
  ℚ≤→ℤ≤ : {(a , b) (c , d) : ℤ × ℕ₊₁}
          → [ a , b ] ≤ [ c , d ]
          → a ℤ.· ℕ₊₁→ℤ d ℤ.≤ c ℤ.· ℕ₊₁→ℤ b
...
≤-o+-cancel : o ℚ.+ m ≤ o ℚ.+ n → m ≤ n
≤-o+-cancel {o} {m} {n}
  = subst2 _≤_ (+Assoc (- o) o m ∙ cong (ℚ._+ m) (+InvL o) ∙ +IdL m)
                (+Assoc (- o) o n ∙ cong (ℚ._+ n) (+InvL o) ∙ +IdL n) ∘
           ≤-o+ {m = o ℚ.+ m} {n = o ℚ.+ n} {o = - o}

≤-+o-cancel : m ℚ.+ o ≤ n ℚ.+ o → m ≤ n
≤-+o-cancel {m} {o} {n}
  = subst2 _≤_ (sym (+Assoc m o (- o)) ∙ cong (λ x → m ℚ.+ x) (+InvR o) ∙ +IdR m)
                (sym (+Assoc n o (- o)) ∙ cong (λ x → n ℚ.+ x) (+InvR o) ∙ +IdR n) ∘
           ≤-+o {m = m ℚ.+ o} {n = n ℚ.+ o} {o = - o}

<-o+ : m < n → o ℚ.+ m < o ℚ.+ n
<-o+ {m} {n} {o} = subst2 _<_ (+Comm m o) (+Comm n o) ∘ <-+o {m = m} {n = n} {o = o}

<Monotone+ : m < n → o < s → m ℚ.+ o < n ℚ.+ s
<Monotone+ {m} {n} {o} {s} m<n o<s
  = isTrans< (m ℚ.+ o) (n ℚ.+ o) (n ℚ.+ s) (<-+o {m} {n} {o} m<n) (<-o+ {o = n} o<s)
...
≤MonotoneMin : m ≤ n → o ≤ s → ℚ.min m o ≤ ℚ.min n s
≤MonotoneMin {m} {n} {o} {s} m≤n o≤s
  = subst (_≤ ℚ.min n s)
          (sym (ℚ.minAssoc n s (ℚ.min m o)) ∙
           ...
           cong₂ ℚ.min (ℚ.minComm n m ∙ ≤→min m≤n)
                       (ℚ.minComm s o ∙ ≤→min o≤s))
           (min≤ {m = ℚ.min n s} {n = ℚ.min m o})

≤MonotoneMax : m ≤ n → o ≤ s → ℚ.max m o ≤ ℚ.max n s
≤MonotoneMax {m} {n} {o} {s} m≤n o≤s
  = subst (ℚ.max m o ≤_)
          (sym (ℚ.maxAssoc m o (ℚ.max n s)) ∙
           ...
           cong₂ ℚ.max (≤→max m≤n) (≤→max o≤s))
          (≤max {m = ℚ.max m o} {n = ℚ.max n s})

≡Weaken≤ : m ≡ n → m ≤ n
≡Weaken≤ {m} {n} m≡n = subst (m ≤_) m≡n (isRefl≤ m)

≤→≯ : m ≤ n → ¬ (m > n)
≤→≯ {m} {n} m≤n n<m = isIrrefl< n (subst (n <_) (isAntisym≤ m n m≤n (<Weaken≤ n<m)) n<m)

≮→≥ : ¬ (m < n) → m ≥ n
≮→≥ {m} {n} m≮n with discreteℚ m n
... | yes m≡n = ≡Weaken≤ (sym m≡n)
...
... | no 0≮m | no 0≮n = ⊥.rec (≤→≯ (≤Monotone+ (≮→≥ 0≮m) (≮→≥ 0≮n)) 0<m+n)
