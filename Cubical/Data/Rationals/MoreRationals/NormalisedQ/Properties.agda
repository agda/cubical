...
·ᵘ-def x y d d' =
  ...
  where
    ...
    step1 = ℕ₊₁→ℤ (1+ predℕ (gcdℕ (absℤ x) (suc d-1)))
    step1' = pos (gcdℕ (absℤ x) (suc d-1))
    step1≡1' = ℕ₊₁→ℤ-gcd-def (absℤ x) d-1
    step3 = 1+ predℕ (gcdℕ (absℤ x) (suc d-1))
    step2 = ℕ₊₁→ℤ (1+ predℕ (gcdℕ (absℤ y) (suc d-1')))
    step2' = pos (gcdℕ (absℤ y) (suc d-1'))
    step2≡2' = ℕ₊₁→ℤ-gcd-def (absℤ y) d-1'
    step4 = 1+ (predℕ (gcdℕ (absℤ y) (suc d-1')))
    step5 : (step1 ℤ.· ((↥ p) ℤ.· (↥ q))) ℤ.· step2 ≡ x ℤ.· y
    step5 = (cong₂ (λ u v → u ℤ.· ((↥ p) ℤ.· (↥ q)) ℤ.· v) step1≡1' step2≡2' ∙
      (ℤ.a'bc'd≡ab'cd step1' (↥ p) (↥ q) step2') ∙
      cong (λ u → u ℤ.· ((↥ q) ℤ.· step2')) (ℤ·Comm step1' (↥ p)) ∙
      cong₂ (λ a b → a ℤ.· b) (sym x≡) (sym y≡))
    step6 = ℕ₊₁.a'bc'd≡ab'cd step3 (↧₊₁ p) (↧₊₁ q) step4 ∙
...
+ᵘ-def x y d d' = step1 ∙ cong₂ (λ u v → [ u , v ]) (step2 ∙ step3) step4
  where
    ...
    over = ((↥ p) ℤ.· (↧ q)) ℤ.+ ((↥ q) ℤ.· (↧ p))
    under = (↧₊₁ p) ·₊₁ (↧₊₁ q)
    xy = pos (gcdℕ (absℤ x) (suc d-1)) ℤ.· pos (gcdℕ (absℤ y) (suc d-1'))
    step1 = sym (·[]CancelL (1+ predℕ (gcdℕ (absℤ x) (suc d-1)) ·₊₁
      1+ predℕ (gcdℕ (absℤ y) (suc d-1'))))
    step2 = cong (λ u → u  ℤ.· over)
      ((·ℕ₊₁→ℤ-distr (1+ predℕ (gcdℕ (absℤ x) (suc d-1)))
       (1+ predℕ (gcdℕ (absℤ y) (suc d-1')))) ∙
      (cong₂ (λ a b → a ℤ.· b)
       (ℕ₊₁→ℤ-gcd-def (absℤ x) d-1) (ℕ₊₁→ℤ-gcd-def (absℤ y) d-1')))
    step3 : xy ℤ.· over ≡ (x ℤ.· ℕ₊₁→ℤ d') ℤ.+ (y ℤ.· ℕ₊₁→ℤ d)
    step3 =
      ℤ·DistR+ xy ((↥ p) ℤ.· (↧ q)) ((↥ q) ℤ.· (↧ p)) ∙
      cong₂ (λ u v → u ℤ.+ v)
       ((ℤ.ab'cd≡ca'db (pos (gcdℕ (absℤ x) (suc d-1)))
        (pos (gcdℕ (absℤ y) (suc d-1'))) (↥ p) (↧ q)) ∙
        cong₂ (λ u w → u ℤ.· w)
         (sym (↥·gcd-lemma x d-1)) (sym (↧·gcd-lemma y d-1')))
       ((ℤ.ab'cd≡cb'da (pos (gcdℕ (absℤ x) (suc d-1)))
        (pos (gcdℕ (absℤ y) (suc d-1'))) (↥ q) (↧ p)) ∙
        cong₂ (λ u w → u ℤ.· w)
         (sym (↥·gcd-lemma y d-1')) (sym (↧·gcd-lemma x d-1)))
    step4 = sym (ℕ₊₁.ab'cd≡ac'bd (1+ predℕ (gcdℕ (absℤ x) (suc d-1))) (↧₊₁ p)
       (1+ predℕ (gcdℕ (absℤ y) (suc d-1'))) (↧₊₁ q)) ∙
      cong₂ (λ u v → u ·₊₁ v)
       (·₊₁-comm (1+ predℕ (gcdℕ (absℤ x) (suc d-1))) (↧₊₁ p))
       (·₊₁-comm (1+ predℕ (gcdℕ (absℤ y) (suc d-1'))) (↧₊₁ q)) ∙
      cong₂ (λ u v → u ·₊₁ v)
       (sym (↧₊₁·gcd-lemma x d-1)) (sym (↧₊₁·gcd-lemma y d-1'))
...
·ZeroR : ∀ x → x · 0ℚ ≡ 0ℚ
·ZeroR x = ↥a≡0→a≡0ℚ (cong ↥_ (cong₂ (λ u v → [ u , v ])
  (ℤ.·AnnihilR (↥ x)) refl))

isIntegralℚ : {p q : ℚ} → ¬ p ≡ 0ℚ → p · q ≡ 0ℚ → q ≡ 0ℚ
isIntegralℚ p@{(z , n) , c} q@{(z' , n') , c'} ¬p0 pq0 =
  ↥a≡0→a≡0ℚ (isIntegralℤ z z' zz0 nz0)
  where
    p' = sym [ c ]Def
    zz0 = [z,n]≡0→z≡0 (sym (cong₂ (λ u v → u · v) p' (sym [ c' ]Def) ∙
...
+Assoc x y z =
  ...
  where
    ...
    mid = ℤ+Assoc ((↥ x) ℤ.· (ℕ₊₁→ℤ (↧₊₁ y) ℤ.· ℕ₊₁→ℤ (↧₊₁ z)))
     ...
     (ℤ.ab'c≡ac'b (↥ y) (ℕ₊₁→ℤ (↧₊₁ z)) (ℕ₊₁→ℤ (↧₊₁ x)))
     ((sym (ℤ·Assoc (↥ z) (ℕ₊₁→ℤ (↧₊₁ y)) (ℕ₊₁→ℤ (↧₊₁ x)))) ∙
     (ℤ.a'bc≡a'cb (↥ z) (ℕ₊₁→ℤ (↧₊₁ y)) (ℕ₊₁→ℤ (↧₊₁ x))))

+IdL : ∀ x → 0ℚ + x ≡ x
+IdL q@((z , n) , copr) =  cong₂ (λ a b → [ a , b ])
   (sym (ℤ.+Comm (z ℤ.· 1) 0) ∙ ℤ.·IdR z) (·₊₁-identityˡ (1+ n)) ∙ [ copr ]Def

+IdR : ∀ x → x + 0ℚ ≡ x
+IdR x = +Comm x 0ℚ ∙ +IdL x

0-p≡-p : ∀ p → 0ℚ - p ≡ - p
0-p≡-p p@((z , n) , copr) = +IdL (- p)

y-x-comm : ∀ x y → (- x) + y ≡ y - x
y-x-comm x y = +Comm (- x) y

+InvR : ∀ x → x - x ≡ 0ℚ
+InvR x = +overSameDenominator' {x} {(- x)} {↧₊₁ x} refl (sym (↧₊₁-neg x)) ∙
  (cong (λ a → [ a , (↧₊₁ x) ]) (↥x+↥-x≡0 x)) ∙ ([0,n]≡0 (↧₊₁ x))

+InvL : ∀ x → (- x) + x ≡ 0ℚ
+InvL x = y-x-comm x x ∙ +InvR x
...
-x≡-1·x : ∀ x → (- x) ≡ -1ℚ · x
-x≡-1·x x@((pos zero , n) , c) = sym (·IdL ((pos zero , n) , c))
-x≡-1·x x@((pos (suc m) , n) , c) = sym (·IdL ((negsuc m , n) , c))
-x≡-1·x x@((negsuc m , n) , c) = sym (·IdL ((pos (suc m) , n) , c))
...
-DistrL· : ∀ x y → - (x · y) ≡ (- x) · y
-DistrL· x y =
  -x≡-1·x (x · y) ∙ ·Assoc -1ℚ x y ∙ cong (λ u → u · y) (sym (-x≡-1·x x))
...
instance
  ...
  nonZero-1/q' {(pos (suc m) , n) , c} ⦃ nz ⦄ = tt
  nonZero-1/q' {(negsuc m , n) , c} ⦃ nz ⦄ = tt

nonZero-1/q : (q : ℚ) {{nz : NonZero q}} → NonZero (1ℚ / q)
nonZero-1/q q =
  subst (λ z → z) (cong NonZero (sym (·IdL (1/ q)))) (nonZero-1/q' {q})

nonZero-p/q : (p q : ℚ) → {{nz : NonZero p}}{{nz' : NonZero q}} → NonZero (p / q)
nonZero-p/q p q {{nz}}{{nz'}} = subst (λ z → z)
...
1/q≡1/q : (q : ℚ) → {{nz : NonZero q}} → 1ℚ / q ≡ 1/ q
1/q≡1/q q {{nz}} = ·IdL (1/ q)

1/q≡[] : (q : ℚ) → {{nz : NonZero q}} →
  (1/ q) {{nz}} ≡ [ (ℤ.sign (↥ q)) ℤ.· ℕ₊₁→ℤ (↧₊₁ q) , (abs↥₊₁ q) {{nz}} ]
1/q≡[] ((pos (suc m) , n) , c) ⦃ nz ⦄ = sym (normalise-coprime (symGCD c))
1/q≡[] ((negsuc m , n) , c) ⦃ nz ⦄ = sym (cong -_ (normalise-coprime (symGCD c)))

1/-subst : {a b : ℚ} → (ab : a ≡ b) {{nz : NonZero a}}{{nz' : NonZero b}} →
  (1/ a) {{nz}} ≡ (1/ b) {{nz'}}
1/-subst {a}{b} ab {{nz}} {{nz'}} = 1/q≡[] a {{nz}} ∙ cong₂ (λ u v → [ u , v ])
  (cong (λ u → ℤ.sign (↥ u) ℤ.· ℕ₊₁→ℤ (↧₊₁ u)) ab)
   (abs↥₊₁-cong a b {{nz}}{{nz'}} ab) ∙ sym (1/q≡[] b {{nz'}})

1/-subst* : {a b : ℚ} → (ab : a ≡ b) → {{nz : NonZero a}} →
  1/ a ≡ (1/ b) {{substNonZero {{nz}} ab}}
1/-subst* {a}{b} ab {{nz}} = let nz' = substNonZero {{nz}} ab in 1/q≡[] a {{nz}} ∙
  cong₂ (λ u v → [ u , v ])
   (cong (λ u → ℤ.sign (↥ u) ℤ.· ℕ₊₁→ℤ (↧₊₁ u)) ab)
...
/-invol : (q : ℚ) {{nz : NonZero q}}{{nz' : NonZero (1ℚ / q)}} →
  (1ℚ / (1ℚ / q)) ≡ q
/-invol q {{nz}}{{nz'}} = 1/q≡1/q (1ℚ / q) {{nz'}} ∙
  (1/-subst (1/q≡1/q q {{nz}})
  {{nz'}}{{nonZero-1/q'}}) ∙ /-invol' q

-- specialises one of the instances of /-invol for convenience
/-invol* : (q : ℚ) {{nz : NonZero q}} →
  (1ℚ / (1ℚ / q)) {{nonZero-1/q q}} ≡ q
/-invol* q {{nz}} = /-invol q {{nz}} {{nonZero-1/q q}}

1/normalise-def : ∀ m n {{nz : NonZero (normalise (suc m) (1+ n))}} →
  (1/ normalise (suc m) (1+ n)) {{nz}} ≡ normalise (suc n) (1+ m)
1/normalise-def m n {{nz}} = 1/-subst p≡p' {{nz}}{{tt}} ∙ ↥↧₋₁a≡↥↧₋₁b→a≡b step
  where
...
1/-p≡-1/p : (p : ℚ) {{np : NonZero p}}{{np' : NonZero (- p)}} →
  1/ (- p) ≡ - (1/ p)
1/-p≡-1/p p@((pos (suc m) , n) , c) = refl
1/-p≡-1/p p@((negsuc m , n) , c) = refl

-- specialises NonZero (- p) instance
1/-p≡-1/p* : (p : ℚ) {{np : NonZero p}} →
  (1/ (- p)) {{ -nonZero p}} ≡ - ((1/ p) {{np}})
1/-p≡-1/p* p {{np}} = 1/-p≡-1/p p {{np}}{{ -nonZero p}}
...
module 1/-helpers where
  ...
  1/possuc·negsuc {m}{m'}{n}{n'} c c' {{npq}} = (sym (1/-subst -pp'≡ {{nnpp'}}{{npq}})) ∙
    ...
    (cong (λ u → ((1/ p) {{tt}}) · u) (sym (1/-p≡-1/p p' {{tt}}{{tt}})))
    where
      p : ℚ ; p' : ℚ ; pp' = p · p' ; -p' : ℚ
  ...
  1/negsuc·negsuc {m}{m'}{n}{n'} c c' {{npq}} =
    1/-subst (-x·-y≡x·y p p') {{npq}} {{npp'}} ∙
    1/possuc·possuc {m}{m'}{n}{n'} c c' {{npp'}} ∙
    sym (-x·-y≡x·y ((1/ p) {{tt}}) ((1/ p') {{tt}})) ∙
    cong₂ (λ u v → u · v) (sym (1/-p≡-1/p p {{tt}}{{tt}}))
     (sym (1/-p≡-1/p p' {{tt}}{{tt}}))
    where
      p : ℚ ; -p : ℚ ; p' : ℚ ; -p' : ℚ
      p = ((pos (suc m) , n) , c) ; -p = (((negsuc m) , n) , c)
...
p·1/q≡p/q : (p q : ℚ) → {{nz : NonZero q}} → p · (1ℚ / q) ≡ p / q
p·1/q≡p/q p q ⦃ nz ⦄ = cong (λ u → p · u) (1/q≡1/q q)

·/Assoc : (p q r : ℚ) → {{nz : NonZero r}} → p · (q / r) ≡ (p · q) / r
·/Assoc p q r ⦃ nz ⦄ = cong (λ u → p · (q · u)) (sym (1/q≡1/q r)) ∙
  ·Assoc p q (1ℚ / r) ∙ p·1/q≡p/q (p · q) r

0/q≡0 : (q : ℚ) {{nz : NonZero q}} → 0ℚ / q ≡ 0ℚ
0/q≡0 q {{nz}} = sym (p·1/q≡p/q 0ℚ q) ∙ (·ZeroL (1ℚ / q))

p/p≡1 : (p : ℚ) → {{nz : NonZero p}} → p / p ≡ 1ℚ
p/p≡1 p@((pos (suc n) , d-1) , c) ⦃ nz ⦄ =
...
p/p≡1 p@((negsuc n , d-1) , c) ⦃ nz ⦄ =
  -x·-y≡x·y ((pos (suc n) , d-1) , c) (((pos (suc d-1)) , n) , symGCD c) ∙
  p/p≡1 ((pos (suc n) , d-1) , c) {{tt}}

1/pq≡1/p·1/q : (p q : ℚ) →
  {{np : NonZero p}}{{nq : NonZero q}}{{npq : NonZero (p · q)}} →
  (1/ (p · q)) ≡ (1/ p) · (1/ q)
1/pq≡1/p·1/q p@((pos (suc m) , n) , c) q@((pos (suc m') , n') , c')
  ⦃ np ⦄ ⦃ nq ⦄ ⦃ npq ⦄ = 1/possuc·possuc c c' {{npq}}
1/pq≡1/p·1/q p@((pos (suc m) , n) , c) q@((negsuc m' , n') , c')
  ⦃ np ⦄ ⦃ nq ⦄ ⦃ npq ⦄ = 1/possuc·negsuc c c' {{npq}}
1/pq≡1/p·1/q p@((negsuc m , n) , c) q@((pos (suc m') , n') , c')
  ⦃ np ⦄ ⦃ nq ⦄ ⦃ npq ⦄ = 1/negsuc·possuc c c' {{npq}}
1/pq≡1/p·1/q p@((negsuc m , n) , c) q@((negsuc m' , n') , c')
  ⦃ np ⦄ ⦃ nq ⦄ ⦃ npq ⦄ = 1/negsuc·negsuc c c' {{npq}}

1/pq≡1/p·1/q* : (p q : ℚ) →
  {{np : NonZero p}}{{nq : NonZero q}} →
  (1/ (p · q)) {{·nonZero p q}} ≡ (1/ p) · (1/ q)
1/pq≡1/p·1/q* p q {{np}}{{nq}} = 1/pq≡1/p·1/q p q {{np}}{{nq}}{{·nonZero p q}}

p/pq≡1/q : (p q : ℚ) → {{np : NonZero p}}{{nq : NonZero q}}
  {{npq : NonZero (p · q)}} → (p / (p · q)) {{npq}} ≡ 1ℚ / q
p/pq≡1/q p q {{np}}{{nq}}{{npq}} =
  cong (λ u → p · u) (1/pq≡1/p·1/q p q {{np}}{{nq}}{{npq}}) ∙
  ·Assoc p (1/ p) (1/ q) ∙ cong (λ u → u · 1/ q) (p/p≡1 p)

p/pq≡1/q* : (p q : ℚ) → {{np : NonZero p}}{{nq : NonZero q}}
  → (p / (p · q)) {{·nonZero p q}} ≡ 1ℚ / q
p/pq≡1/q* p q {{np}}{{nq}} = p/pq≡1/q p q {{np}}{{nq}}{{·nonZero p q}}

pq/rs≡p/qxr/s : (p q r s : ℚ) →
  {{nr : NonZero r}} {{ns : NonZero s}}{{nrs : NonZero (r · s)}} →
  (p · q) / (r · s) ≡ (p / r) · (q / s)
pq/rs≡p/qxr/s p q r s ⦃ nr ⦄ ⦃ ns ⦄ ⦃ nqr ⦄ =
  cong (λ u → (p · q) · u) (1/pq≡1/p·1/q r s) ∙ ab'cd≡ac'bd p q (1/ r) (1/ s)

pq/rs≡p/qxr/s* : (p q r s : ℚ) →
  {{nr : NonZero r}} {{ns : NonZero s}} →
  ((p · q) / (r · s)) {{·nonZero r s}} ≡ (p / r) · (q / s)
pq/rs≡p/qxr/s* p q r s ⦃ nr ⦄ ⦃ ns ⦄ =
  pq/rs≡p/qxr/s p q r s {{nr}}{{ns}}{{·nonZero r s}}

·/CancelL : (p q r : ℚ) {{np : NonZero p}}{{nr : NonZero r}}
  {{npr : NonZero (p · r)}} → (p · q) / (p · r) ≡ q / r
·/CancelL p q r {{np}}{{nr}}{{npr}} = pq/rs≡p/qxr/s p q p r {{np}}{{nr}}{{npr}} ∙
  cong (λ u → u · (q / r)) (p/p≡1 p) ∙ ·IdL (q / r)
...
1/p/q≡q/p p q {{np}}{{nq}}{{npq}} =
  (cong (λ u → u / (p / q)) (sym (p/p≡1 q))) ∙ (cong (λ u → (q · 1/ q) · u)
   (1/-subst {p / q} {p / q} refl {{npq}}{{·nonZero p (1/ q)}})) ∙
  ·/CancelR* (1/ q) q p {{nonZero-1/q' {q}}}{{np}}
