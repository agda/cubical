...
ℚ : Type
ℚ = Σ[ (a , b) ∈ (ℤ × ℕ) ] areCoprime (absℤ a , suc b)
...
↥≡0→≡0ℚ : {a : ℚ} → ↥ a ≡ 0 → a ≡ 0ℚ
↥≡0→≡0ℚ {a@((z , d-1) , c)} a0 = ℚ-unique₋₁ a0 (ℕ.injSuc (zeroCoprime' copr))
  where
...
≡0→↥≡0 :{a : ℚ} → a ≡ 0ℚ → ↥ a ≡ 0
≡0→↥≡0 {((n , d-1) , c)} a0 = cong ↥_ a0

≡-of-↥≡0-of-↥≡0 : ∀ {a} {b} → ↥ a ≡ 0 → ↥ b ≡ 0 → a ≡ b
≡-of-↥≡0-of-↥≡0 {a@((n , d-1) , c)} {b@((n' , d-1') , c')} a0 b0 =
  (↥≡0→≡0ℚ a0) ∙ sym (↥≡0→≡0ℚ b0)

-- equality depends on numerator and denominator
↥↧₊₁-injective : ∀ {a}{b} → ↥↧₊₁ a ≡ ↥↧₊₁ b → a ≡ b
↥↧₊₁-injective {a}{b} ab = ℚ-unique₊₁ (cong fst ab) (cong snd ab)
...
NonZero→≢0 p@{(pos (suc m) , n) , c} tt =
  converse ≡0→↥≡0 (λ x → snotz (injPos x))
NonZero→≢0 p@{(negsuc m , n) , c} tt =
  converse ≡0→↥≡0 (λ x → ℤ.negsucNotpos m 0 x)
...
normalise-coprime {m}{n} copr = ↥↧₊₁-injective (↥↧₊₁-normalise m n copr)
...
[0,-]≡0 : ∀ n → [ 0 , n ] ≡ 0ℚ
[0,-]≡0 n = normaliseZero n

[ℤ,-]≡0→ℤ≡0 : ∀ {z}{n} → [ z , n ] ≡ 0ℚ → z ≡ 0
[ℤ,-]≡0→ℤ≡0 {pos zero} {n} zn0 = refl
[ℤ,-]≡0→ℤ≡0 {pos (suc m)} {n} zn0 =
  ⊥.elim {A = λ x → pos (suc m) ≡ pos 0} (normaliseNonZero m n zn0)
[ℤ,-]≡0→ℤ≡0 {negsuc m} {n} zn0 =
  ⊥.elim {A = λ x → negsuc m ≡ pos 0}
   (normaliseNonZero m n ((sym neg-involutive) ∙ (cong -_ zn0) ))

≢0→¬[ℤ,-]≡0 : {z : ℤ}{n : ℕ₊₁} → ¬ (z ≡ pos zero) → ¬ ([ z , n ] ≡ 0ℚ)
≢0→¬[ℤ,-]≡0 {z}{n} nz = converse [ℤ,-]≡0→ℤ≡0 nz

numSucNonZero : ∀ {k n+} → NonZero [ pos (suc k) , n+ ]
numSucNonZero {k}{n+} =
  ≢0→NonZero (≢0→¬[ℤ,-]≡0 {pos (suc k)}{n+} (λ x → snotz (injPos x)))
...
≃→-≃- : ∀ {p}{q} → ↥ p ℤ.· ↧ q ≡ ↥ q ℤ.· ↧ p →
  ↥ (- p) ℤ.· ↧ (- q) ≡ ↥ (- q) ℤ.· ↧ (- p)
≃→-≃- {p}{q} pq = cong₂ (λ a b → a ℤ.· b) (↥-neg p) (↧-neg q) ∙
  sym (-DistL· (↥ p) (↧ q)) ∙ cong -ℤ_ pq ∙ -DistL· (↥ q) (↧ p) ∙
  sym (cong₂ (λ a b → a ℤ.· b) (↥-neg q) (↧-neg p))

↥↧≡-of-cross≡-pos : ∀ {m}{n}{m'}{n'}{c}{c'} →
  ↥ ((pos m , n) , c) ℤ.· ↧ ((pos m' , n') , c') ≡
  ↥ ((pos m' , n') , c') ℤ.· ↧ ((pos m , n) , c) →
  ↥↧ ((pos m , n) , c) ≡ ↥↧ ((pos m' , n') , c')
↥↧≡-of-cross≡-pos {zero} {n} {zero} {n'} {c} {c'} pq =
  cong ↥↧ (0-id {n}{c}) ∙ sym (cong ↥↧ (0-id {n'}{c'}))
↥↧≡-of-cross≡-pos {zero} {n} {suc m'} {n'} {c} {c'} pq =
...
≃→≡ {p@((pos m ,  n) , c)} {q@((pos m' ,  n') , c')} (*≡* pq) =
  ↥↧-injective (↥↧≡-of-cross≡-pos {c = c}{c' = c'} pq)
...
≃→≡ {p@((negsuc m , n) , c)} {q@((negsuc m' , n') , c')} (*≡* pq) =
  let npq = (↥↧≡-of-cross≡-pos {c = c}{c' = c'} (≃→-≃- {p}{q} pq))
