...
module Cubical.Data.DescendingList.DescendingList
  (A : Type₀)
...
cons≡ {x} p = J P d p
 where
  P : (y : A) → x ≡ y → Type₀
  P y p = {u v : DL} {r : x ≥ᴴ u} {s : y ≥ᴴ v} → u ≡ v → cons x u r ≡ cons y v s
  d : P x refl
  d {u} q = J Q c q
   where
    Q : (v : DL) → u ≡ v → Type₀
    Q v q = {r : x ≥ᴴ u} {s : x ≥ᴴ v} → cons x u r ≡ cons x v s
    c : Q u refl
...
insert-cons : (x : A) (u : DL) (r : x ≥ᴴ u) → insert x u ≡ cons x u r
...
insert-∷ : (x : A) (u : DL) → toFMSet (insert x u) ≡ x ∷ toFMSet u
insert-∷ x [] = refl
