{-# OPTIONS --safe #-}
module Cubical.Foundations.Equiv.NewHalfAdjoint where
...
isHAEquiv.linv (isEquiv→isHAEquiv f h) x = sym (retIsEq h x)
isHAEquiv.rinv (isEquiv→isHAEquiv f h) x = secIsEq h x
isHAEquiv.coh (isEquiv→isHAEquiv f h) x i j = commSqIsEq h x (~ i) j

Iso→isHAEquiv : {A : Type ℓ} {B : Type ℓ'} (f : Iso A B) → isHAEquiv (Iso.fun f)
Iso→isHAEquiv f = isEquiv→isHAEquiv (Iso.fun f) (isoToIsEquiv f)

foo : {A : Type ℓ} {B : Type ℓ'} (f : A → B) (h : isHAEquiv f)
    → isEquiv→isHAEquiv f (isHAEquiv→isEquiv f h) ≡ h
foo f h = {!!}

isPropisHAEquiv : {A : Type ℓ} {B : Type ℓ'} (f : A → B) → isProp (isHAEquiv f)
isPropisHAEquiv f = isPropRetract (isHAEquiv→isEquiv f) (isEquiv→isHAEquiv f) (foo f) (isPropIsEquiv f)
