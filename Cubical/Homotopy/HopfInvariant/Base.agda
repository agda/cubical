...
module _ (n : ℕ) (f : S₊∙ (3 +ℕ n +ℕ n) →∙ S₊∙ (2 +ℕ n)) where
  private
    ...
    preSphere→H : (g : (S₊ (suc (suc n)) → coHomK 2+n)) → H → coHomK (2 +ℕ n)
    preSphere→H g (inl x) = 0ₖ _
    preSphere→H g (inr x) = g x -ₖ g north
    preSphere→H g (push a i) = h23 a i
      where
  ...
  Iso.leftInv HIPSphereCohomIso =
                  ...
                  (λ gn gtt →
                   trRec (isProp→isOfHLevelSuc n (squash₂ _ _))
                     (λ r → cong ∣_∣₂ (funExt λ {
                                 ...
                                 ; (push a i)
                                   → sphereElim _
                                        {A = λ a → PathP (λ i → preSphere→H (λ x → g (inr x)) (push a i) ≡ g (push a i))
                                                       (sym gtt)
    ...
    push-helper : (g : HopfInvariantPush n (fst f) → coHomK (suc (suc n)))
     ...
     → hLevelTrunc (suc n) (PathP (λ i → preSphere→H (λ x → g (inr x)) (push north i) ≡ g (push north i))
             (sym gtt) ((λ i → g (inr (fst f north)) -ₖ gn i) ∙ rUnitₖ _ (g (inr (fst f north)))))
    push-helper g gtt gn = isConnectedPathP _ (isConnectedPath _ (isConnectedKn _) _ _) _ _ .fst
