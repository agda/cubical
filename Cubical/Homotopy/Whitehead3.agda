...
module Cubical.Homotopy.Whitehead3 where
...
IsoSucSphereSusp∙ : (n : ℕ) → inv (IsoSucSphereSusp n) north ≡ ptSn (suc n)
IsoSucSphereSusp∙ zero = refl
IsoSucSphereSusp∙ (suc n) = refl

Iso1 : ∀ {ℓ} {X : Pointed ℓ}
  → Iso ((join S¹ S¹ , inl base) →∙ X)
         (S₊∙ 3 →∙ X)
Iso1 = Σ-cong-iso-fst (domIso (IsoSphereJoin 1 1))

open import Cubical.Homotopy.Group.Base
funcheck : ∀ {ℓ} {X : Pointed ℓ}
           → (join S¹ S¹ , inl base) →∙ X
           → (join S¹ S¹ , inl base) →∙ X
           → (join S¹ S¹ , inl base) →∙ X
funcheck {X = X} f g =
  Iso.inv Iso1 (∙Π (Iso.fun Iso1 f) (Iso.fun Iso1 g))

IsoS³ : Iso (S₊ 3) (join S¹ S¹)
fun IsoS³ north = inl base
fun IsoS³ south = inr base
fun IsoS³ (merid north i) = push base base i
fun IsoS³ (merid south i) = push base base i
fun IsoS³ (merid (merid base j) i) =
  (push base (loop j) ∙ sym (push (loop j) (loop j)) ∙ push (loop j) base) i
fun IsoS³ (merid (merid (loop k) j) i) =
  hcomp (λ r → λ {(i = i0) → {!x!}
                   ; (i = i1) → {!!}
                   ; (j = i0) → {!!}
                   ; (j = i1) → {!!}
...
funcheck2 : ∀ {ℓ} {X : Pointed ℓ}
           (f g : (join S¹ S¹ , inl base) →∙ X)
           → funcheck f g ≡ {!(x : S¹) → (p : fst (funcheck f g) (inr x) ≡ snd X) → p ≡ refl!}
funcheck2 f g =
  ΣPathP ((funExt (λ { (inl base) → {!!}
                     ; (inl (loop i)) → {!!}
                     ; (inr x) → {!!}
                     ; (push a b i) → {!!}}))
         , {!!})

-- fst (fun (Iso1 {X = X}) (f , p)) = Iso.fun (domIso (IsoSphereJoin 1 1)) f
-- snd (fun (Iso1 {X = X}) (f , p)) = p
-- fst (inv (Iso1 {X = X}) (g , p)) = {!!}
-- snd (inv (Iso1 {X = X}) (g , p)) = {!!}
-- rightInv (Iso1 {X = X}) (g , p) = {!!}
-- leftInv (Iso1 {X = X}) (f , p) = {!!}

-- joinS¹S¹-comp : ∀ {ℓ} {X : Pointed ℓ}
--        → ((join S¹ S¹ , inl base) →∙ X)
--        → ((join S¹ S¹ , inl base) →∙ X)
--        → ((join S¹ S¹ , inl base) →∙ X)
-- fst (joinS¹S¹-comp {X = X} (f , fp) (g , gp)) (inl x) = f (inl x)
-- fst (joinS¹S¹-comp {X = X} (f , fp) (g , gp)) (inr x) = g (inr x)
-- fst (joinS¹S¹-comp {X = X} (f , fp) (g , gp)) (push a b i) = {!f (push a b i)!}
-- snd (joinS¹S¹-comp {X = X} (f , fp) (g , gp)) = {!!}
...
-- snd ([_∣_] {n = zero} {m = m} f g) = {!!}
-- snd ([_∣_] {n = suc n} {m = zero} f g) = {!refl!}
-- snd ([_∣_] {n = suc n} {m = suc m} f g) = {!!}

-- trr : (S₊∙ (suc (suc 1))) →∙ S₊∙ (suc (suc zero))
-- trr = [_∣_]' {n = 0} {m = 0} (idfun∙ _) (idfun∙ _)
-- open import Cubical.Homotopy.Group.Base
-- open import Cubical.Homotopy.HopfInvariant.HopfMap

-- gen₁ = [_∣_]-pre {X = S₊∙ 2} (idfun∙ _) (idfun∙ _)
-- gen₂ = {!!}

-- joinLoop : ∀ {ℓ} (X : Pointed ℓ)
--          → fst X → fst X
--          → Path (join (typ X) (typ X)) (inl (pt X)) (inl (pt X))
-- joinLoop X a b = (push (pt X) (pt X) ∙ sym (push a (pt X))) ∙∙ push a b ∙∙ sym (push (pt X) b) 

-- joinS¹comp : ∀ {ℓ} {X : Pointed ℓ}
--   (x y : (join S¹ S¹ , inl base) →∙ X)
--   → (join S¹ S¹ , inl base) →∙ X
-- fst (joinS¹comp {X = X} f g) (inl x) = pt X --fst f (inl x)
-- fst (joinS¹comp {X = X} f g) (inr x) = pt X
-- fst (joinS¹comp f g) (push a b i) =
--     (sym (snd f) ∙∙ cong (fst f) (joinLoop (S₊∙ 1) a b) ∙∙ snd f
--   ∙ (sym (snd g) ∙∙ cong (fst g) (joinLoop (S₊∙ 1) a b) ∙∙ snd g)) i
-- snd (joinS¹comp f g) = refl

-- lem123 : (x : join S¹ S¹)
--   → gen₁ x
--   ≡ fst (joinS¹comp (fst ∘ (hopfS¹.TotalSpaceHopfPush→TotalSpace ∘ (Iso.inv IsoTotalHopf')) , refl)
--           (fst ∘ (hopfS¹.TotalSpaceHopfPush→TotalSpace ∘ (Iso.inv IsoTotalHopf')) , refl)) x
-- lem123 (inl x) = refl
-- lem123 (inr x) = refl
-- lem123 (push a b i) = {!invEq (hopfS¹.μ-eq' base) b!}
--   where
--   *⁻ : S¹ → S¹
--   *⁻ a = invEq (hopfS¹.μ-eq' a) base
--   lem1 : (a : S¹) → (*⁻ a) * a ≡ base
--   lem1 a = commS¹ (invEq (hopfS¹.μ-eq' a) base) a
--          ∙ secEq (hopfS¹.μ-eq' a) base

--   pushLem : (a b : S¹) → cong (fst ∘ hopfS¹.TotalSpaceHopfPush→TotalSpace ∘ inv IsoTotalHopf') (push a b)
--                        ≡ merid (invEq (hopfS¹.μ-eq' a) b)
--   pushLem a b = cong (cong (fst ∘ hopfS¹.TotalSpaceHopfPush→TotalSpace))
--                      (λ _ → (push (a , invEq (hopfS¹.μ-eq' a) b)
--                      ∙ cong inr (secEq (hopfS¹.μ-eq' a) b)))
--               ∙ cong-∙ (fst ∘ hopfS¹.TotalSpaceHopfPush→TotalSpace) (push (a , invEq (hopfS¹.μ-eq' a) b))
--                                                                     (cong inr (secEq (hopfS¹.μ-eq' a) b))
--               ∙ sym (rUnit _)
--               ∙ (λ i → merid (invEq (hopfS¹.μ-eq' a) b))

--   lem₁ : (cong (fst ∘ hopfS¹.TotalSpaceHopfPush→TotalSpace ∘ inv IsoTotalHopf'))
--          (joinLoop (S₊∙ 1) a b)
--        ≡ {!refl!}
--   lem₁ = cong-∙∙ (fst ∘ hopfS¹.TotalSpaceHopfPush→TotalSpace ∘ inv IsoTotalHopf')
...
--                  (push a b)
--                  (sym (push base b))
--       ∙∙ (λ i → (cong-∙ (fst ∘ hopfS¹.TotalSpaceHopfPush→TotalSpace ∘ inv IsoTotalHopf')
--                        (push base base) (sym (push a base)) ∙ cong₂ _∙_ (pushLem base base) (cong sym (pushLem a base))) i
--               ∙∙ pushLem a b i
--               ∙∙ sym (pushLem base b i))
--       ∙∙ ((λ i → (merid base ∙ sym (merid (*⁻ a))) -- (σ(a) * σ(a * -b) * σ(-b)) = σ (2a - 2b)
--                ∙∙ merid (invEq (hopfS¹.μ-eq' a) b)
--                ∙∙ (sym (merid b)))
--       ∙∙ {!!}
--       ∙∙ {!!})

--   F = _∨→_ (idfun∙ (S₊∙ 2)) (idfun∙ (S₊∙ 2))

--   cool : cong gen₁ (push a b) ≡ (σ (S₊∙ 1) b ∙∙ (refl ∙ refl) ∙∙ σ (S₊∙ 1) a)
--   cool = cong-∙∙ F (λ i → inr (σ (S₊∙ 1) b i))
--                    (sym (push tt))
--                    (λ i → inl (σ (S₊∙ 1) a i))
--        ∙ λ i → cong-∙ (F ∘ inr) (merid b) (sym (merid base)) i
--             ∙∙ refl ∙ refl
--             ∙∙ cong-∙ (F ∘ inl) (merid a) (sym (merid base)) i



-- -- trr≡hopfMap : trr ≡ ∙Π (HopfMap' , refl) (HopfMap' , refl)
-- -- trr≡hopfMap =
...
-- --   lem2 : (p : inl base ≡ inl base)
-- --     → cong (((idfun∙ _) ∨→ (idfun∙ _)) ∘
-- --               (W {A = S₊∙ 1} {B = S₊∙ 1})) p
-- --     ≡ cong (fst ∘ hopfS¹.TotalSpaceHopfPush→TotalSpace
-- --                       ∘ Iso.inv IsoTotalHopf') (p ∙ p)
-- --   lem2 p = {!!}

-- --   help₁ : (a : S₊ 2) → cong (fst trr) (merid a) ≡ cong HopfMap' (merid a) ∙ cong HopfMap' (merid a)
-- --   help₁ a = (λ i j → ((idfun∙ _) ∨→ (idfun∙ _))
-- --               (W {A = S₊∙ 1} {B = S₊∙ 1}
-- --           (inv (IsoSphereJoin 1 1) (merid a j))))
-- --          ∙∙ {! !}
-- --          ∙∙ λ i → (λ j → hopfS¹.TotalSpaceHopfPush→TotalSpace
-- --                       (Iso.inv IsoTotalHopf'
-- --                         (Iso.inv (IsoSphereJoin 1 1) (merid a j))) .fst)
-- --                  ∙ (λ j → hopfS¹.TotalSpaceHopfPush→TotalSpace
-- --                       (Iso.inv IsoTotalHopf'
-- --                         (Iso.inv (IsoSphereJoin 1 1) (merid a j))) .fst)

-- --   -- help : (a : S₊ 2) → cong (fst trr) (merid a) ≡
-- --   --      (cong HopfMap' (merid a ∙ sym (merid north)) ∙ refl)
-- --   --    ∙ (cong HopfMap' (merid a ∙ sym (merid north)) ∙ refl)
-- --   -- help a = {!cong (fst trr) (merid a)!}
-- --   --        ∙ {!HopfMap' (merid a ?)!}
-- --   --        ∙ cong₂ _∙_ ((rUnit (cong HopfMap' (merid a))
-- --   --                    ∙ cong (cong HopfMap' (merid a) ∙_) (cong sym (sym lem))
-- --   --                   ∙ sym (cong-∙ HopfMap' (merid a) (sym (merid north))))
-- --   --                   ∙ rUnit (cong HopfMap' (merid a ∙ sym (merid north))))
-- --   --                   (((rUnit (cong HopfMap' (merid a))
-- --   --                    ∙ cong (cong HopfMap' (merid a) ∙_) (cong sym (sym lem)))
-- --   --                   ∙ sym (cong-∙ HopfMap' (merid a) (sym (merid north))))
-- --   --                   ∙ rUnit (cong HopfMap' (merid a ∙ sym (merid north))))
-- --   --   where
-- --   --   lem : cong HopfMap' (merid north) ≡ refl
-- --   --   lem = {!!}
