...
module _ (A B : Type) (a₀ : A) (b₀ : B) where
  ...
  PushoutPushout : Iso ((Pushout {A = Susp A × B} fst fst))
                       (Susp A × Susp B)
  fun PushoutPushout (inl x) = x , north
  fun PushoutPushout (inr x) = x , north
  fun PushoutPushout (push (x , b) i) = x , φB b i
  inv PushoutPushout (a , north) = inl a
  inv PushoutPushout (a , south) = inr a
  inv PushoutPushout (a , merid a₁ i) = push (a , a₁) i
  rightInv PushoutPushout (a , north) = refl
  rightInv PushoutPushout (a , south) = {!!}
  rightInv PushoutPushout (a , merid b i) = {!!}
  leftInv PushoutPushout (inl x) = refl
  leftInv PushoutPushout (inr x) = push (x , b₀)
  leftInv PushoutPushout (push a i) = {!!}
