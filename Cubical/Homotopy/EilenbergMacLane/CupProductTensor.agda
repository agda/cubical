  ...
  private
    ...
    assoc₀₁ₗ (suc l) =
      assocInd zero 1 (suc l) _ _
        λ x y z → help (assoc₀₁ₗ l) z y x
                 ∙ sym (preSubstFunLoop (+'-assoc zero (suc zero) (suc l))
    ...
    assoc₀ₘₗ n (suc l) ind =
      assocInd zero (2 + n) (suc l) _ _
        λ x y z → lem x y z ∙ sym (preSubstFunLoop (+'-assoc zero (suc (suc n)) (suc l))
                           (swapFun zero (suc (suc n)) (suc l)
                             (x ⌣ₖ (∣ y ∣ ⌣ₖ EM-raw'→EM L' (suc l) z))))
      where
      lem : (x : G) (y : EM-raw' H' (suc (suc n))) (z : EM-raw' L' (suc l))
         → (_⌣ₖ_ {n = 0} {m = suc (suc n)} x ∣ y ∣) ⌣ₖ EM-raw'→EM L' (suc l) z
          ≡ swapFun zero (suc (suc n)) (suc l)
             (x ⌣ₖ (∣ y ∣ ⌣ₖ EM-raw'→EM L' (suc l) z))
      lem x north z = refl
      lem x south z = refl
    ...
    assoc₁₀ₗ =
      ...
      l₃ n ind (emloop-raw g i) y z = flipSquare help i
        ...
        help = cong (EM→ΩEM+1 (suc (suc n)))
                ...
                ∙ preSubstFunLoop (+'-assoc zero zero (suc (suc n))) (swapFun zero zero (suc (suc n))
                   (g ⌣ₖ (y ⌣ₖ ∣ z ∣))))
    ...
    assocₙ₀ₗ (suc n) zero ind =
      assocInd (suc (suc n)) zero zero _ _
        λ x y z → lem x y z
         ∙ sym (preSubstFunLoop (+'-assoc (suc (suc n)) zero zero) ((swapFun (suc (suc n)) zero zero _)))
      where
    ...
    assocₙ₀ₗ (suc n) (suc zero) ind =
      assocInd (suc (suc n)) zero (suc zero) _ _
       λ x y z →
           lem x y z
         ∙ sym (preSubstFunLoop (+'-assoc (suc (suc n)) zero (suc zero)) (swapFun (suc (suc n)) zero (suc zero) _))
      where
    ...
    assocₙ₀ₗ (suc n) (suc (suc l)) ind =
      assocInd (suc (suc n)) zero (suc (suc l)) _ _
        λ x y z →
            lem x y z
          ∙ sym (preSubstFunLoop (+'-assoc (suc (suc n)) zero (suc (suc l)))
                  (swapFun (suc (suc n)) zero (suc (suc l)) _))
      where
      lem : (x : EM-raw' G' (suc (suc n))) (y : H)
      ...
      lem (emloop-raw g i) y z = flipSquare help i
        ...
        help = sym (EM→ΩEM+1-distrₙ₀ zero z
             ...
             ∙ cong (EM→ΩEM+1 1)
                ...
                ∙ preSubstFunLoop (+'-assoc zero (suc zero) zero) (swapFun zero (suc zero) zero
    ...
    assocₙₘ₀ zero (suc m) ind =
      assocInd (suc zero) (suc (suc m)) zero _ _
        λ x y z →
            lem x y z
          ∙ sym (preSubstFunLoop (+'-assoc (suc zero) (suc (suc m)) zero) (swapFun (suc zero) (suc (suc m)) zero _))
      where
      ...
      lem (emloop-raw g i) y z = flipSquare help i
        ...
        help = sym (EM→ΩEM+1-distrₙ₀ (suc m) z
             ...
             ∙ cong (EM→ΩEM+1 (suc (suc m)))
                   (assocConvert ind g (EM-raw'→EM _ _ y) z
                  ∙ preSubstFunLoop (+'-assoc zero (suc (suc m)) zero) (swapFun zero (suc (suc m)) zero
    ...
    assocₙₘ₀ (suc n) zero ind =
      assocInd (suc (suc n)) (suc zero) zero _ _
        λ x y z →
            lem x y z
          ∙ sym (preSubstFunLoop (+'-assoc (suc (suc n)) (suc zero) zero) (swapFun (suc (suc n)) (suc zero) zero _))
      where
    ...
    assocₙₘ₀ (suc n) (suc m) ind =
      assocInd (suc (suc n)) (suc (suc m)) zero _ _
        λ x y z →
           lem x y z
         ∙ sym (preSubstFunLoop (+'-assoc (suc (suc n)) (suc (suc m)) zero) (swapFun (suc (suc n)) (suc (suc m)) zero _))
      where
    ...
    assocₙₘₗ₊ zero m l ind =
      assocInd (suc zero) (suc m) (suc l) _ _
        λ x y z →
            lem x y z
          ∙ sym (preSubstFunLoop (+'-assoc (suc zero) (suc m) (suc l)) (swapFun (suc zero) (suc m) (suc l) _))
      where
