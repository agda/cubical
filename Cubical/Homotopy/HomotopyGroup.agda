...
flipΩIsopres· : {ℓ : Level} {A : Pointed ℓ} (n : ℕ)
                      ...
                      ≡ (Iso.inv (flipΩIso (suc n)) f) ∙ (Iso.inv (flipΩIso (suc n)) g)
flipΩIsopres· {A = A} n f g i =
    transp (λ j → flipΩPath {A = A} n (~ i ∧ ~ j) .snd ≡ flipΩPath n (~ i ∧ ~ j) .snd) i
...
module _ {ℓ : Level} {A : Pointed ℓ} where
  ...
  suspension-pres∙ n f g = ΣPathP ((funExt main) , refl)
    ...
    looper-help (suc n) f g (merid a j) i = help i j
      ...
      help = (cong₂ _∙_ (cong (sym (snd f) ∙∙_∙∙ snd f)
                        ...
                        ((cong (sym (snd g) ∙∙_∙∙ snd g)
                               (cong-∙ (fst g) (merid a) (sym (merid (ptSn _))))))
    ...
    main (merid a i₁) j = help j i₁
      where
      help : fst (∙Π f g) a ≡ cong (fst (∙Π (suspension→ n f) (suspension→ n g))) (merid a)
      help = looper-help n f g a
        ∙ cong₂ _∙_ (sym (cong-∙ (fst (suspension→ n f)) (merid a) (sym (merid (ptSn _)))
                       ∙∙ cong (fst f a ∙_) (cong sym (snd f))
                       ∙∙ sym (rUnit _)))
                    (sym (cong-∙ (fst (suspension→ n g)) (merid a) (sym (merid (ptSn _)))
                       ∙∙ cong (fst g a ∙_) (cong sym (snd g))
                       ∙∙ sym (rUnit _)))
        ∙ λ i → rUnit (cong (fst (suspension→ n f)) (merid a ∙ sym (merid (ptSn _)))) i
               ∙ rUnit (cong (fst (suspension→ n g)) (merid a ∙ sym (merid (ptSn _)))) i

  suspension←filler : (n : ℕ) → (S₊∙ (suc (suc n)) →∙ A) → I → I → I → fst A
  ...
  Iso.leftInv (suspensionIso n) f =
    ...
    sideCube : Cube (λ j k → snd f j (~ k))
                               (λ j k → fst (suspension→ n f) (rCancel' (merid (ptSn _)) j k))
                    (λ r k → suspension→ n f .fst
                               (compPath-filler' (merid (ptSn _)) (sym (merid (ptSn _))) r k))
                    (λ _ _ → pt A)
                    (λ r j → snd f j (~ r)) λ _ _ → pt A
    sideCube r j k =
      hcomp (λ i → λ {(r = i0) → snd f j (~ k ∨ ~ i)
                     ; (r = i1) → fst (suspension→ n f) (rCancel-filler' (merid (ptSn _)) (~ i) j k)
...
SphereMap→Ω {A = A} (suc n) (f , p) =
  Iso.inv (flipΩIso _) (SphereMap→Ω {A = Ω A} n (Iso.inv (suspensionIso _) (f , p)))
...
Ω→SphereMap : {ℓ : Level} {A : Pointed ℓ} (n : ℕ) → (fst ((Ω^ (suc n)) A)) → (S₊∙ (suc n) →∙ A)
fst (Ω→SphereMap {A = A} zero f) base = pt A
fst (Ω→SphereMap {A = A} zero f) (loop i₁) = f i₁
snd (Ω→SphereMap {A = A} zero f) = refl
Ω→SphereMap {A = A} (suc n) f = Iso.fun (suspensionIso _) (Ω→SphereMap n (Iso.fun (flipΩIso _) f))
...
SphereMap→Ω→SphereMap zero (f , p) = ΣPathP (funExt fstp , λ i j → p (~ i ∨ j))
  where
  fstp : (x : _) → _ ≡ f x
  fstp base = sym p
  fstp (loop i) j = doubleCompPath-filler (sym p) (cong f loop) p (~ j) i
SphereMap→Ω→SphereMap (suc n) f =
     cong (Iso.fun (suspensionIso n) ∘ Ω→SphereMap n) (Iso.rightInv (flipΩIso (suc n))
          (SphereMap→Ω n (Iso.inv (suspensionIso n) f)))
  ∙∙ cong (Iso.fun (suspensionIso n)) (SphereMap→Ω→SphereMap n (Iso.inv (suspensionIso n) f))
...
IsoSphereMapΩ : ∀ {ℓ} {A : Pointed ℓ} (n : ℕ)
  → Iso (S₊∙ n →∙ A) (fst ((Ω^ n) A))
Iso.fun (IsoSphereMapΩ {A = A} zero) (f , p) = f false
fst (Iso.inv (IsoSphereMapΩ {A = A} zero) a) false = a
fst (Iso.inv (IsoSphereMapΩ {A = A} zero) a) true = pt A
snd (Iso.inv (IsoSphereMapΩ {A = A} zero) a) = refl
Iso.rightInv (IsoSphereMapΩ {A = A} zero) a = refl
Iso.leftInv (IsoSphereMapΩ {A = A} zero) (f , p) =
  ΣPathP ((funExt (λ { false → refl ; true → sym p})) , λ i j → p (~ i ∨ j))
Iso.fun (IsoSphereMapΩ {A = A} (suc n)) = SphereMap→Ω n
Iso.inv (IsoSphereMapΩ {A = A} (suc n)) = Ω→SphereMap n
Iso.rightInv (IsoSphereMapΩ {A = A} (suc n)) = Ω→SphereMap→Ω n
Iso.leftInv (IsoSphereMapΩ {A = A} (suc n)) = SphereMap→Ω→SphereMap n

-- The iso is structure preserving
IsoSphereMapΩ-pres∙Π : ∀ {ℓ} {A : Pointed ℓ} (n : ℕ) (f g : S₊∙ (suc n) →∙ A)
             → SphereMap→Ω n (∙Π f g) ≡ SphereMap→Ω n f ∙ SphereMap→Ω n g
IsoSphereMapΩ-pres∙Π zero f g = sym (rUnit _)
IsoSphereMapΩ-pres∙Π (suc n) f g =
     cong (Iso.inv (flipΩIso (suc n)))
          (cong (SphereMap→Ω n) (suspension←-pres∙ n f g)
            ∙ SphereMap→Ωpres∙ n (suspension← n f) (suspension← n g))
   ∙ (flipΩIsopres· n (SphereMap→Ω n (suspension← n f)) (SphereMap→Ω n (suspension← n g)))

-- It is useful to define the ``Group Structure'' on (S₊∙ n →∙ A) before doing it on π'
-- These will be the equivalents of the usual groupoid laws on Ω A.
...
fst (∙Π-rUnit {A = A} {n = suc n} f i) south = (sym (snd f) ∙ cong (fst f) (merid (ptSn (suc n)))) i
...
fst (∙Π-lUnit {n = suc n} f i) south = (sym (snd f) ∙ cong (fst f) (merid (ptSn _))) i
fst (∙Π-lUnit {n = suc n} f i) (merid a j) = help i j
  ...
  help : PathP (λ i → snd f (~ i) ≡ (sym (snd f) ∙ cong (fst f) (merid (ptSn (suc n)))) i)
               ...
               (cong (fst f) (merid a))
  help =
    (cong (_∙ ((sym (snd f)) ∙∙ (cong (fst f) (merid a ∙ sym (merid (ptSn (suc n))))) ∙∙ snd f))
           (sym (rUnit refl))
    ∙ sym (lUnit _))
...
fst (∙Π-rCancel {A = A} {n = zero} f i) (loop j) = rCancel (sym (snd f) ∙∙ cong (fst f) loop ∙∙ snd f) i j
...
∙Π-assoc {n = n} f g h =
  ...
  ∙∙ cong (Ω→SphereMap n) (SphereMap→Ωpres∙ n f (∙Π g h)
                ∙∙ cong (SphereMap→Ω n f ∙_) (SphereMap→Ωpres∙ n g h)
                ∙∙ ∙assoc (SphereMap→Ω n f) (SphereMap→Ω n g) (SphereMap→Ω n h)
                ∙∙ cong (_∙ SphereMap→Ω n h) (sym (SphereMap→Ωpres∙ n f g))
                ∙∙ sym (SphereMap→Ωpres∙ n (∙Π f g) h))
...
∙Π-comm {A = A} {n = n} f g =
  ...
  ∙∙ cong (Ω→SphereMap (suc n)) (SphereMap→Ωpres∙ (suc n) f g
  ∙∙ EH _ _ _
  ∙∙ sym (SphereMap→Ωpres∙ (suc n) g f))
