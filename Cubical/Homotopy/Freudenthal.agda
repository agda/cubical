...
depEq north = S₊ 2
depEq south = S₊ 2
depEq (merid north i) = S₊ 2
depEq (merid south i) = S₊ 2
depEq (merid (merid a j) i) = {!!}
  where
  testmap : hLevelTrunc 4 S² → hLevelTrunc 4 S²
  testmap = {!!} 

  testmap≡Idfun : (x : S²) → surf j i ≡ x
  testmap≡Idfun base m = surf j (i ∨ m)
  testmap≡Idfun (surf k l) m = {!!}

  test2 : I → I → I → I
  test2 = {!λ {(i = i0) → ? ; (i = i1) → ?}!}

-- S²→Set : ∀ {B : S² → Type ℓ} → B base → ((x : S²) → isSet (B x)) → (x : S²) → B x
-- S²→Set bc hlev base = bc
-- S²→Set bc hlev (surf i j) = isOfHLevel→isOfHLevelDep 2 hlev bc bc (λ _ → bc) (λ _ → bc) surf i j

-- meridGen3 : (x : Susp S²) (y : S²) → ∥ north ≡ x ∥ 2
-- meridGen3 x = S²→Set (help x) λ _ → isOfHLevelTrunc 2
--   where
--   help : (x : Susp S²) → ∥ north ≡ x ∥ 2
--   help north = ∣ refl ∣
--   help south = ∣ merid base ∣
--   help (merid a i) = help2 a i
--     where
--     help2 : (a : S²) → PathP (λ i → ∥ north ≡ merid a i ∥ 2) ∣ refl ∣ ∣ merid base ∣
--     help2 = S²→Set (λ i → ∣ ((λ j → merid base (i ∧ j))) ∣)
--                     λ _ → isOfHLevelPathP 2 (isOfHLevelTrunc 2) ∣ refl ∣ ∣ merid base ∣


-- meridGen2 : (x : Susp S²) (y : S²) → ∥ north ≡ x ∥ 2
-- meridGen2 north y = ∣ refl ∣
-- meridGen2 south y = ∣ merid y ∣
-- meridGen2 (merid base i) base = ∣ (λ j → merid base (i ∧ j)) ∣
-- meridGen2 (merid (surf i₁ i₂) i) base = {!!}
-- meridGen2 (merid a i) (surf i₁ i₂) = {!!}

-- open import Cubical.Functions.Embedding
-- open import Cubical.Functions.Surjection
-- m2 : isEmbedding (cong {A = S²} {B = λ x → Path (Susp S²) north south} {x = base} {y = base} merid) -- (x : Susp S²) (y z : S²) → ∥ Path {!!} {!merid !} {!!} ∥ 2
-- m2  = λ w x → {!!}

-- mermap : S¹ → ∥ Path (Path (S₊ 3) north south) (merid north) (merid south) ∥ 3
-- mermap a = ∣ cong merid (merid a) ∣

-- mermaphelp : S¹ → (x : Susp S¹) →  ∥ Path (Path (S₊ 3) north south) (merid north) {!!} ∥ 2
-- mermaphelp x north = ∣ refl ∣
-- mermaphelp x south = ∣ cong merid (merid x) ∣
-- mermaphelp base (merid base i) = ∣ (λ j → merid (merid base (i ∧ j))) ∣
-- mermaphelp (loop i₁) (merid base i) = {!!}
-- mermaphelp x (merid (loop i₁) i) = {!!}

-- open import Cubical.Data.Int
-- mermaphelp3 : Int → (x : S¹) →  ∥ Path (Path (Path (S₊ 3) north south) (merid north) (merid south)) (cong merid (merid base)) (cong merid (merid x)) ∥ 1
-- mermaphelp3 a base = ∣ cong (cong merid) (cong merid (intLoop a)) ∣
-- mermaphelp3 a (loop i) = {!!}

-- test3 : {!(x : S¹) (Int ) → ∥ Path (Path (Path (S₊ 3) north south) (merid north) (merid south)) (cong merid (merid base)) (cong merid (merid x)) → ?!}
-- test3 = {!!}



-- open import Cubical.HITs.PropositionalTruncation renaming (∣_∣ to ∣_∣₋₁ ; ∥_∥ to ∥_∥₋₁) 
-- isSurjectiveCongM : (x : Susp S²) (y z : S²) → isSurjection (cong {x = y} {y = z} (meridGen3 x))
-- isSurjectiveCongM = suspToPropRec {!!} {!!} λ {base → λ {base → λ b → ∣ {!!} , {!meridGen3 north base!} ∣₋₁ ; (surf k l) → {!!}} ; (surf i j) → {!!}}

-- meridGen : (x : S₊ 3) (y : S₊ 2) → ∥ north ≡ x ∥ 1
-- meridGen north y = ∣ refl ∣
-- meridGen south y = ∣ merid y ∣
-- meridGen (merid a i) y =
--   isOfHLevel→isOfHLevelDep 1 {A = S₊ 3} {B = λ x → ∥ north ≡ x ∥ 1}
--     (λ _ → isOfHLevelTrunc 1) ∣ refl ∣ ∣ merid y ∣ (merid a) i

-- test2 : (x : Susp S²) (p : north ≡ x) → Σ[ y ∈ S² ] meridGen3 x y ≡ ∣ p ∣
-- test2 x = J (λ x p → Σ[ y ∈ S² ] meridGen3 x y ≡ ∣ p ∣) (base , refl)

-- conclusion : (p : Path (Susp S²) north south) → ∥ fiber merid p ∥ 1
-- conclusion p = Trunc.rec (isOfHLevelTrunc 1) (λ id → ∣ (fst (test2 south p)) , id ∣) (Iso.fun (PathIdTruncIso 1) (snd (test2 south p)))
...
-- --     helper2 = (λ i j k → transport (λ l → code (surf j k l)) ∣ base ∣) ∙∙ {!test2 (λ i j → ∣ surf i j ∣)!} ∙∙ {!!} {- (λ i → (λ i i₁ → transport (λ i₂ → Iso.inv mainIso cons i i₁ i₂) ∣ base ∣))
...
-- --         test2 : ∀ {ℓ} {A : Type ℓ} {x y : A} {p q : x ≡ y} (R : x ≡ x) → q ∙∙ sym p ∙∙ R ∙∙ p ∙∙ sym q ≡ R
-- --         test2 = {!!}
...
-- --                 → Iso (hLevelTrunc ((suc n) + (suc n)) (typ A))
-- --                       (hLevelTrunc ((suc n) + (suc n)) (typ (Ω (Susp (typ A) , north))))
-- -- FreudenthalIso zero A iscon = connectedTruncIso _ (σ 0 {A = A} iscon) (isConnectedσ _ iscon)
