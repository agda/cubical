...
joinS¹S¹→S³ (push a b i) = (merid (S¹×S¹→S² a b) ∙ sym (merid north)) i
...
S³→SuspS²' north = north
S³→SuspS²' south = north
S³→SuspS²' (merid x i) = (merid (S²→S²' x) ∙ sym (merid base)) i

joinS¹S¹→SuspS²' : join S¹ S¹ → Susp S²'
joinS¹S¹→SuspS²' x = S³→SuspS²' (joinS¹S¹→S³ x)

β₂ : ℤ
β₂ = g10 (g9 (g8 {!!})) -- λ i j → {!f7 -- joinS¹S¹→SuspS²' (η₂ (push (loop i0) (loop j) ?))!}))
  where
  _→∙_ : Type 
  _→∙_ = {!Σ[ f ∈ (Pointed₀ → Pointed₀) ] ?!}

  m : Ω (Susp∙ S²') .fst → ∥ S²'∙ ∥₂∙ .fst
  m f7 = {!!}
  
  t : Ω (Ω² (Susp∙ S²')) .fst
  t i j k =
    hcomp (λ r → λ { (i = i0) → {!!}
                     ; (i = i1) → {!η₂ (push (loop i) (loop j) k)!}
                     ; (j = i0) → {!north!}
                     ; (j = i1) → {!north!}
                     ; (k = i0) → north
                     ; (k = i1) → north
                     ...
                     (joinS¹S¹→SuspS²'
      (η₂ (push (loop i) (loop j) k)))

  h : Ω² ∥ S²'∙ ∥₂∙ .fst -- Ω² ∥ S²' ∥₂ .fst
  h i j = f7 λ k → t i j k
