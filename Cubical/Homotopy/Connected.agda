...
isConnectedSubtr : ∀ {ℓ ℓ'} {A : Type ℓ} {B : Type ℓ'} (n m : ℕ) (f : A → B) → isHLevelConnectedFun (n + m) f → isHLevelConnectedFun n f
...
module elim {ℓ ℓ' : Level} {A : Type ℓ} {B : Type ℓ'} (f : A → B) (n : ℕ) where
  ...
  hasSectionPrecompose : ∀ {ℓ'''} (P : B → HLevel ℓ''' n)
                      → isEquiv (λ(s : (b : B) → P b .fst) → s ∘ f)
                      → hasSection λ(s : (b : B) → P b .fst) → s ∘ f
  hasSectionPrecompose P record { equiv-proof = eqPf } =
                       (λ g → (eqPf g) .fst .fst) , (λ b  → (eqPf b) .fst .snd)
  ...
  isConnectedPrecompose P→sect b = (c n P→sect b) , λ y → sym (fun n P→sect b y)
    where
    P : (n : ℕ) → (∀ {ℓ} (P : B → HLevel ℓ n) → hasSection (λ(s : (b : B) → P b .fst) → s ∘ f)) → B → Type _
    P n s b = hLevelTrunc n (fiber f b)

    c : (n : ℕ) → (∀ {ℓ} (P : B → HLevel ℓ n) → hasSection (λ(s : (b : B) → P b .fst) → s ∘ f)) → (b : B) → hLevelTrunc n (fiber f b)
    c n s = (s λ b → (hLevelTrunc n (fiber f b) , isOfHLevelTrunc _)) .fst λ a → ∣ a , refl ∣

    fun : (n : ℕ) → (s : (∀ {ℓ} (P : B → HLevel ℓ n) → hasSection λ(s : (b : B) → P b .fst) → s ∘ f)) → (b : B) (w : (hLevelTrunc n (fiber f b))) → w ≡ c n s b
    fun zero s b w = isOfHLevelSuc zero (isOfHLevelTrunc _) w (c zero s b)
    fun (suc n) s b = Trunc.elim (λ x → isOfHLevelPath (suc n) (isOfHLevelTrunc _) x (c (suc n) s b)) λ a → transport eqtyp c* b (fst a) (snd a)
        where
        ...
        eqtyp =
            isoToPath
              (iso (λ g b a → J (λ b p → ∣ a , p ∣ ≡ c (suc n) s b) (g a))
                   ...
                   (λ h → funExt λ b
                        → funExt λ a
                        → funExt (J (λ b x → (J (λ b₂ p → ∣ a , p ∣ ≡ c (suc n) s b₂) (h (f a) a (λ _ → f a)) x ≡ h b a x))
                                          (JRefl (λ b₂ p → ∣ a , p ∣ ≡ c (suc n) s b₂) (h (f a) a (λ _ → f a)))))
                   λ h → funExt λ a → JRefl (λ b₁ p → ∣ a , p ∣ ≡ c (suc n) s b₁) (h a))
...
indMapEquiv→conType : ∀ {ℓ} {A : Type ℓ} (n : ℕ) → (∀ {ℓ'} (B : HLevel ℓ' n) → isEquiv (λ (b : (fst B)) → λ (a : A) → b)) → isHLevelConnected n A
indMapEquiv→conType {A = A} n BEq = transport (λ i → isContr (hLevelTrunc n (typeToFiber A tt (~ i))))
                                   (elim.isConnectedPrecompose (λ _ → tt) n
                                                               (λ {ℓ'} P → elim.hasSectionPrecompose (λ _ → tt) n P
                                                                                                      (compEquiv ((λ Q → Q tt) , isoToIsEquiv (helper P))
                                                                                                                 (_ , BEq (P tt)) .snd))
                                                               tt)
  where
  helper : ∀ {ℓ'} → (P : Unit → HLevel ℓ' n) → Iso ((b : Unit) → fst (P b)) (fst (P tt))
...
isHLevelConnectedPoint2 n {A = A} a connMap = indMapEquiv→conType _ λ B → isoToIsEquiv (theIso B)
  ...
  module _  {ℓ' : Level} (B : HLevel ℓ' (suc n))
    ...
    helper f = equiv-proof (elim.isEquivPrecompose (λ _ → a) n (λ a2 → (f a2 ≡ f a) , isOfHLevelPath' n (snd B) (f a2) (f a)) connMap) (λ _ → refl) .fst .fst
...
connectedTruncIso {A = A} {B = B} (suc n) f con = g
  ...
  Iso.leftInv g = helper
    where
    abstract
      helper : _
      helper = Trunc.elim (λ x → isOfHLevelPath (suc n) (isOfHLevelTrunc _) _ _)
                               λ a → cong (map fst) (con (f a) .snd  ∣ a , refl ∣)
  Iso.rightInv g = helper
    where
    abstract
      helper : _
      helper = Trunc.elim (λ x → isOfHLevelPath (suc n) (isOfHLevelTrunc _) _ _)
...
connectedTruncEquiv2 : ∀ {ℓ} {A B : Type ℓ} (n m : ℕ) (f : A → B) → Σ[ x ∈ ℕ ] n + x ≡ m
                   → isHLevelConnectedFun m f
                   → hLevelTrunc n A  ≃ hLevelTrunc n B
connectedTruncEquiv2 {A = A} {B = B} n m f (x , pf) con =
  connectedTruncEquiv n f (isConnectedSubtr n x f (transport (λ i → isHLevelConnectedFun (pf (~ i)) f) con))
...
inrConnected {A = A} {B = B} {C = C} n f g iscon = elim.isConnectedPrecompose inr n λ P → (λ  l → k P l) , λ b → refl
  where
  module _ {ℓ : Level} (P : (Pushout f g) → HLevel ℓ n)
          ...
          k (inl x) = elim.hasSectionPrecompose f n Q (elim.isEquivPrecompose f n Q iscon) .fst fun x
          k (inr x) = h x
          k (push a i) =
            hcomp (λ k → λ { (i = i0) → (elim.hasSectionPrecompose f n Q (elim.isEquivPrecompose f n Q iscon) .snd fun i0 a)
                            ; (i = i1) → transportTransport⁻ (λ j → typ (P (push a j))) (h (g a)) k })
                  (transp (λ j → typ (P (push a (i ∧ j))))
                          (~ i)
                          (elim.hasSectionPrecompose f n Q (elim.isEquivPrecompose f n Q iscon) .snd fun i a ))
...
-- {-
-- {- Lemma 8.6.1. in the HoTT book -}

-- private
--   module helpers (n k : ℕ) (f : A → B) (P : B → HLevel ℓ'' (k + n)) (l : (a : A) → fst (P (f a))) (iscon : isHLevelConnectedFun n f) where
--       F : _
--       F = λ(s : (b : B) → P b .fst) → s ∘ f

--       fiberPath1 : Iso (fiber F l) (Σ[ g ∈ ((b : B) → P b .fst) ] ((a : A) → g (f a) ≡ l a))
--       Iso.fun fiberPath1 p = (p .fst) , (funExt⁻ (p .snd))
--       Iso.inv fiberPath1 p = (p .fst) , (funExt (p .snd))
--       Iso.rightInv fiberPath1 p = refl
--       Iso.leftInv fiberPath1 p = refl

--       throwAboutHelper : ∀{ℓ} {A : Type ℓ} {a b c : A} → (r : b ≡ c) (p : a ≡ b) (q : a ≡ c) → (p ≡ q ∙ (sym r) ) ≡ (q ≡ p ∙ r)
--       throwAboutHelper  {a = a} {b = b} r p q = (λ i → rUnit p i ≡ q ∙ sym r) ∙
--                                                  (λ i → p ∙ (λ j → r (j ∧ i)) ≡ q ∙ λ j → r (i ∨ (~ j))) ∙
--                                                  (λ i → p ∙ r ≡ rUnit q (~ i)) ∙ symId _ _
--         where
--         symId : ∀{ℓ} {A : Type ℓ} (a b : A) → (a ≡ b) ≡ (b ≡ a)
--         symId a b = isoToPath symiso
--           where
--           symiso : Iso (a ≡ b) (b ≡ a)
--           Iso.fun symiso = sym
--           Iso.inv symiso = sym
--           Iso.rightInv symiso a = refl
--           Iso.leftInv symiso a = refl

--       throwAbout : (g h : (Σ[ g ∈ ((b : B) → P b .fst) ] ((a : A) → g (f a) ≡ l a)))
--                  → (r : (x : B) → (g .fst) x ≡ (h .fst) x)
--                  → Iso (g . snd ≡ (λ x → r (f x) ∙ h .snd x))
--                         ((x : A) → r (f x) ≡ (g .snd x) ∙ sym (h .snd x))
--       Iso.fun (throwAbout (g , p) (h , q) r) id x =
--         transport (λ i → throwAboutHelper (sym (q x)) (p x) (r (f x)) i)
--                   (funExt⁻ id x)
--       Iso.inv (throwAbout (g , p) (h , q) r) id =
--         funExt λ x → transport (λ i → throwAboutHelper (sym (q x)) (p x) (r (f x)) (~ i)) (id x)
--       Iso.rightInv (throwAbout (g , p) (h , q) r) id =
--         funExt λ x → transportTransport⁻ (throwAboutHelper (sym (q x)) (p x) (r (f x))) (id x)
--       Iso.leftInv (throwAbout (g , p) (h , q) r) id j =
--         funExt (λ x → (transport⁻Transport (throwAboutHelper (sym (q x)) (p x) (r (f x))) (λ i → id i x)) j)

--       helper3 : (g h : (Σ[ g ∈ ((b : B) → P b .fst) ] ((a : A) → g (f a) ≡ l a)))
--              → (r : (x : B) → (g .fst) x ≡ (h .fst) x) →
--                 (PathP (λ i → (x : A) → r (f x) i ≡ l x) (g .snd) (h .snd))
--                ≡ ((x : A) → r (f x) ≡ ((g .snd) x) ∙ sym ((h .snd) x))
--       helper3 (g , p) (h , q) r j =
--         hcomp (λ k → λ{ (j = i0) → PathP (λ i → (x : A) → r (f x) i ≡ l x) p λ x → lUnit (q x) (~ k)
--                        ; (j = i1) → isoToPath (throwAbout (g , p) (h , q) r) k})
--                (PathP (λ i → (x : A) → r (f x) (i ∧  ~ j) ≡ l x)
--                       p
--                       λ x → (λ i → r (f x) (~ j ∨ i)) ∙ (q x))

--       iso2 : (g h : (Σ[ g ∈ ((b : B) → P b .fst) ] ((a : A) → g (f a) ≡ l a)))
--            → Iso (Σ[ p ∈ g .fst ≡ h .fst ] PathP (λ i → (a : A) → p i (f a) ≡ l a) (g .snd) (h .snd))
--                   (Σ[ r ∈ ((b : B) → g .fst b ≡ h .fst b) ] r ∘ f ≡ λ a → (g .snd a) ∙ (h .snd a) ⁻¹)
--       Iso.fun (iso2 (g , p) (h , q)) (funid , pathp) = funExt⁻ funid , funExt (transport (helper3 (g , p) (h , q) λ x i → funid i x) pathp)

--       Iso.inv (iso2 (g , p) (h , q)) (r , id) = (funExt r) , transport (sym (helper3 (g , p) (h , q) r)) λ x i → id i x
--       Iso.rightInv (iso2 (g , p) (h , q)) a i =  (fst a) , (funExt (transportTransport⁻ (helper3 (g , p) (h , q) (a .fst)) (funExt⁻ (snd a)) i))
--       Iso.leftInv (iso2 (g , p) (h , q)) a i = fst a , transport⁻Transport (helper3 (g , p) (h , q) (λ x i → a .fst i x )) (a .snd) i


-- isConnectedFamily : (n k : ℕ) (f : A → B) (P : B → HLevel ℓ'' (k + n))
--                  → isHLevelConnectedFun n f
--                  → isHLevelTruncatedFun k (λ(s : (b : B) → P b .fst) → s ∘ f)
-- isConnectedFamily {A = A} {B = B} n zero f P iscon = equiv-proof (elim.isEquivPrecompose f n P iscon)
-- isConnectedFamily {A = A} {B = B} n (suc zero) f P iscon l =
--   transport (λ i → isOfHLevel 1 (isoToPath (helpers.fiberPath1 n 1 f P l iscon) (~ i)))
--             λ a b → transport (λ i → (sym (ua Σ≡) ∙ (isoToPath (helpers.iso2 n 1 f P l iscon a b))) (~ i))
--                                (isConnectedFamily n 0 f
--                                                   (λ x → (a .fst x ≡ b .fst x) , isOfHLevelPath' n (P x .snd) (a .fst x) (b .fst x))
--                                                   iscon
--                                                   (λ r → (a .snd r) ∙ (b .snd r) ⁻¹) .fst )
-- isConnectedFamily {A = A} {B = B} n (suc (suc k)) f P iscon l =
--   transport (λ i → isOfHLevel (2 + k) (isoToPath (helpers.fiberPath1 n (2 + k) f P l iscon) (~ i)))
--             λ a b → transport (λ i → isOfHLevel (suc k) ((sym (ua Σ≡) ∙ (isoToPath (helpers.iso2 n (2 + k) f P l iscon a b))) (~ i)))
--                                (isConnectedFamily n (suc k) f
--                                                   (λ x → (a .fst x ≡ b .fst x) , ((P x) .snd (a .fst x) (b .fst x)))
--                                                   iscon
--                                                   λ r → (a .snd r) ∙ (b .snd r) ⁻¹)
...
-- helper : (n : ℕ) {A B : Pointed₀} →  hLevelTrunc n (fst A) → hLevelTrunc n (fst B) → hLevelTrunc n (A ⋀ B)
-- helper n = Trunc.rec (isOfHLevelΠ _ (λ _ → isOfHLevelTrunc _)) λ a → Trunc.rec (isOfHLevelTrunc _) λ b → ∣ inr (a , b) ∣

-- truncTest : (n : ℕ) {A B : Pointed₀} → (hLevelTrunc n (fst A) , ∣ pt A ∣) ⋀ (hLevelTrunc n (fst B) , ∣ pt B ∣)
--           → hLevelTrunc n (A ⋀ B)
-- truncTest n (inl x) = ∣ inl tt ∣
-- truncTest n {A = A} {B = B} (inr (x , x₁)) = helper n x x₁
-- truncTest n {A = A} {B = B} (push (inl x) i) = helper2 x (~ i)
--   where
--   helper2 : (x : hLevelTrunc n (fst A)) → helper n {A = A} {B = B} x ∣ pt B ∣ ≡ ∣ inl tt ∣
--   helper2 = Trunc.elim (λ x → isOfHLevelPath n (isOfHLevelTrunc n) _ _) λ a i → ∣ push (inl a) (~ i) ∣
-- truncTest n {A = A} {B = B} (push (inr x) i) = helper2 x (~ i)
--   where
--   helper2 : (x : hLevelTrunc n (fst B)) → helper n {A = A} {B = B} ∣ pt A ∣ x ≡ ∣ inl tt ∣
--   helper2 = Trunc.elim (λ x → isOfHLevelPath n (isOfHLevelTrunc n) _ _) λ a i → ∣ push (inr a) (~ i) ∣
-- truncTest n {A = A} {B = B} (push (push tt i₁) i) =  ∣ push (push tt i₁) i ∣

-- hLevelSmash : {A B : Pointed₀} (n : ℕ) → isOfHLevel n ((hLevelTrunc n (fst A) , ∣ pt A ∣) ⋀ (hLevelTrunc n (fst B) , ∣ pt B ∣))
-- hLevelSmash {A = A} {B = B} zero = (inl tt) , {!!}
--   where
--   helper2 : (y : (hLevelTrunc zero (fst A) , ∣ pt A ∣) ⋀ (hLevelTrunc zero (fst B) , ∣ pt B ∣)) → inl tt ≡ y
--   helper2 (inl tt) = refl
--   helper2 (inr (x , x₁)) = push (inr x₁) ∙ (λ i → inr (isOfHLevelTrunc 0 .snd ∣ snd A ∣ (~ i)  , x₁)) ∙ λ i → inr (((isOfHLevelTrunc 0 .snd) x i) , x₁)
--   helper2 (push (inl x) i) = {!!}
--   helper2 (push (inr x) i) = {!!}
--   helper2 (push (push a i₁) i) = {!!}
-- hLevelSmash {A = A} {B = B} (suc n) = {!!}
...
-- -- elim.isConnectedPrecompose (Smash-map (f , fpt) (idfun∙ (C , ptC))) _ λ {ℓ} P → proof.h {ℓ} P , λ b  → funExt (proof.hsection {ℓ} P b) -- elim.isConnectedPrecompose (Smash-map (f , fpt) (idfun∙ (C , ptC))) _ λ {ℓ} P → (λ h → proof.h {ℓ} P h) ,push (inl x)) λ b → funExt λ x → {!!}
--   where
--   module proof {ℓ : Level} (P : (Smash (B , ptB) (C , ptC)) → HLevel ℓ (1 + nc + nf))
--            (d : (x : Smash (A , ptA) (C , ptC)) → P ((Smash-map (f , fpt) (idfun∙ (C , ptC)) ) x) .fst)
--     where
--     test : (c : C) (b : B) → (hLevelTrunc (suc (nc + nf)) (fiber (Smash-map (f , fpt) (idfun∙ (C , ptC))) (proj b c)))
--         ≡ ((hLevelTrunc _ (fiber f b) , conf b .fst) ⋀ {!!})
--     test = {!!}
-- --     F : (c : C) → _
