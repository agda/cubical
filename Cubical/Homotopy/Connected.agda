...
connectedTruncEquiv2 : ∀ {ℓ} {A B : Type ℓ} (n m : ℕ) (f : A → B) → Σ[ x ∈ ℕ ] n + x ≡ m
                   → isHLevelConnectedFun m f
                   → hLevelTrunc n A  ≃ hLevelTrunc n B
connectedTruncEquiv2 {A = A} {B = B} n m f (x , pf) con = connectedTruncEquiv n f (isConnectedSubtr n x f (transport (λ i → isHLevelConnectedFun (pf (~ i)) f) con))
...
inrConnected {A = A} {B = B} {C = C} n f g iscon = elim.isConnectedPrecompose inr n λ P → (λ  k → helpLemmas.k P k) , λ b → refl
  where
  module helpLemmas {ℓ : Level} (P : (Pushout f g) → HLevel ℓ n)
                   (h : (b : B) → typ (P (inr b)))
          where
          Q : A → HLevel _ n
          Q a = (P (inl a))
          ...
          k (inr x) = h x
          k (push a i) = hcomp (λ k → λ { (i = i0) → (elim.hasSectionPrecompose f n Q (elim.isEquivPrecompose f n Q iscon) .snd fun i a)
...
sphereConnected (suc n) =
         transport (λ i → isHLevelConnected (2 + n) ((isoToPath (SuspPushIso {A = S₊ n})) (~ i)))
  ...
  SuspPushIso : ∀ {ℓ} {A : Type ℓ} → Iso (Susp A) (Pushout {A = A} (λ a → tt) λ a → tt)
  Iso.fun SuspPushIso north = inl tt
  Iso.fun SuspPushIso south = inr tt
  Iso.fun SuspPushIso (merid a i) = push a i
  Iso.inv SuspPushIso (inl tt) = north
  Iso.inv SuspPushIso (inr tt) = south
  Iso.inv SuspPushIso (push a i) = merid a i
  Iso.leftInv SuspPushIso north = refl
  Iso.leftInv SuspPushIso south = refl
  Iso.leftInv SuspPushIso (merid a i) = refl
  Iso.rightInv SuspPushIso (inl tt) = refl
  Iso.rightInv SuspPushIso (inr tt) = refl
  Iso.rightInv SuspPushIso (push a i) = refl
