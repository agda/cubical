...
module _ {ℓ ℓ'} (X : MetricSpace ℓ) where 

 -- HasCubesₖ : ℕ → Type (ℓ-max ℓ ℓ')
 -- HasCubesₖ n = (UContMap (ℝCubeMetricSpace n) X) → Cube n A

 -- Has∂Cubesₖ : ℕ → Type (ℓ-max ℓ ℓ')
 -- Has∂Cubesₖ n = (UContMap (ℝCubeMetricSpace n) X) → ∂Cube n A


 -- HasCubesₖ→Has∂Cubesₛₖ : ∀ n → HasCubesₖ n → Has∂Cubesₖ (suc n) 
 -- HasCubesₖ→Has∂Cubesₛₖ = {!!}


 skinAℝCube : ∀ n → (c : UContMap (ℝCubeMetricSpace (suc n)) X)
    → Σ (UContMap (ℝCubeMetricSpace n) X)
         λ f → ∀ 𝕣b → fst f (ℝ∂CubeInj n 𝕣b) ≡
           fst c (ℝ∂CubeInj (suc n) (∂𝕣corner1 n)) 
 skinAℝCube zero c .fst = uContMapConst
   (ℝCubeMetricSpace zero) X
   (fst c (𝕣corner0 (suc zero)))
 skinAℝCube zero c .snd = {!!}
 skinAℝCube (suc n) c = {!!}

 -- skinAℝCubeCorner : ?
 -- skinAℝCubeCorner = ?
 
 skinAℝCubeConst : ∀ n x y →
     (skinAℝCube n ((λ _ → x) , y)) ≡
           (uContMapConst (ℝCubeMetricSpace n) X x , λ _ → refl)
 skinAℝCubeConst zero x y = Σ≡Prop
  (λ _ → isPropΠ λ _ → MetricSpaceStr.is-set (snd X) _ _)
   refl
 skinAℝCubeConst (suc n) x y = {!!}


 module _ (A : Type ℓ') (η : ⟨ X ⟩ → A) where

  record HasNCubes (n : ℕ) : Type (ℓ-max ℓ ℓ') where
   field
    evalIns :
      ∀ (a : ⟨ X ⟩) (c : UContMap (ℝCubeMetricSpace n) X)
       → (∀ 𝕣b → fst c (ℝ∂CubeInj n 𝕣b) ≡ a )
       → (Σ (NCube n → A) λ c → ∀ b → c (boundaryInj b) ≡ η a )
    evalInsConst :
      ∀ a y → evalIns a ((λ _ → a) , y) (λ _ → refl) ≡
                       ((λ _ → η a) , λ _ → refl)
    


   evalInsBd : (UContMap (ℝCubeMetricSpace (suc n)) X)
     → NBoundary (suc n) → A
   evalInsBd f = from-surfⁿ _ n
    (η (fst f ((ℝ∂CubeInj (suc n) (∂𝕣corner1 n)))) ,
     evalIns _ (fst (skinAℝCube n f)) (snd (skinAℝCube n f)))

   evalInsBdConst : ∀ x y b → evalInsBd ((λ _ → x) , y) b ≡ η x
   evalInsBdConst x y b =
    let z =  (evalInsConst x
           (snd (uContMapConst (ℝCubeMetricSpace n) X x)))
    in (cong (from-surfⁿ A n ∘ (η x ,_))
        ( cong (uncurry (evalIns x)) (skinAℝCubeConst n x y) ∙ z)
         ≡$ b)
        ∙ from-surfⁿ-refl A n (η x) b



   data ShS : Type (ℓ-max ℓ ℓ') where
    σ : A → ShS
    hub : (UContMap (ℝCubeMetricSpace (suc n)) X)
        → ShS
    spoke : ∀ bd (b : NBoundary (suc n))
      → hub bd ≡ σ (evalInsBd bd b) -- (fst hc bd b)
    spokeRefl : ∀ x y b b' →
      Square
      (spoke ((λ _ → x) , y) b)
      (cong σ (evalInsBdConst x y b'))
      (spoke ((λ _ → x) , y) b')
      (cong σ (evalInsBdConst x y b))
...
open HasNCubes using (σ; hub; spoke; spokeRefl)
...
module _ {ℓ} (X : MetricSpace ℓ) where
 ...
 Shₖ : ℕ → Type ℓ
 Shₖ-η : ∀ n → ⟨ X ⟩ → Shₖ n
 Shₖ-ceb : ∀ n → HasNCubes X (Shₖ n) (Shₖ-η n) n

 hasNCubesMap : ∀ n (A : Type ℓ) toA (B : Type ℓ) → (a→b : A → B) 
   → HasNCubes X A toA n
   → HasNCubes X B (a→b ∘S toA) n
 hasNCubesMap n A toA B a→b x = w
  where

  module M = HasNCubes x

  w : HasNCubes X B (a→b ∘S toA) n
  w .HasNCubes.evalIns a c x =
    let (f , p) = M.evalIns a c x
    in a→b ∘S f , cong a→b ∘ p 
  w .HasNCubes.evalInsConst a y =
    cong (λ (p , q) → a→b ∘S p , cong a→b ∘ q) (M.evalInsConst a y)
 Shₖ zero = ⟨ X ⟩
 ...
 Shₖ-ceb zero = hasNCubes0 X
 Shₖ-ceb (suc n) = sucCEB _ _ _  (Shₖ-η n) (Shₖ-ceb n)
 ...
 hasAllCubes : ∀ n → HasNCubes X Shape κ n 
 hasAllCubes n =
  let w =  hasNCubesMap n
          (Shₖ n)
          (Shₖ-η n)
          Shape (incl {n = n}) (Shₖ-ceb n)
  in subst (λ κ → HasNCubes X Shape κ n)
 ...
 pathFromℝPath'-refl : ∀ x →
   pathFromℝPath'
    (uContMapConst (ℝCubeMetricSpace 1) X x)
     ≡ refl
 pathFromℝPath'-refl x =
   cong (_∙∙ (sym (spoke (uContMapConst (ℝCubeMetricSpace 1) X x)
      (lid false V.[]))) ∙∙
        (spoke (uContMapConst (ℝCubeMetricSpace 1) X x)
      (lid true V.[])))
         (cong (cong σ)
           (cong sym {!!} ∙ rUnit _))
        -- (cong ((cong σ)) {!!}
        ...
        --    ((rUnit {!!})))
   ∙ sym
    ((cong (cong σ) (lUnit (refl)))
      ◁ PathP→compPathR∙∙ (congP (λ _ → symP)
      (symP (spokeRefl x  (snd (uContMapConst (ℝCubeMetricSpace 1) X x))
          (lid false V.[]) (lid true V.[])))))
          

--  -- pathFromℝPath : (f : UContMap UnitIntervalMetricSpace {!!})
--  --    → {!!}
--  -- pathFromℝPath = {!!}
