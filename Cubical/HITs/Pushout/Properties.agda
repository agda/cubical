...
module _ {ℓA ℓB ℓC ℓD : Level}
  ...
  ⊔²→⊔ (inr (inl x)) = inl (f x)
  ⊔²→⊔ (inr (inr x)) = inr x
  ⊔²→⊔ (inr (push a i)) = push a i
  ⊔²→⊔ (push a i) = inl (f a)

  ⊔→⊔² : A⊔D → A⊔B⊔D
  ⊔→⊔² (inl x) = inl x
  ⊔→⊔² (inr x) = inr (inr x)
  ⊔→⊔² (push a i) = (push (g a) ∙ cong inr (push a)) i
  ...
  ⊔→⊔²→⊔ (push a i) j = help j i
    where
    help : cong ⊔²→⊔ (push (g a) ∙ cong inr (push a)) ≡ push a
    help = cong-∙ ⊔²→⊔ (push (g a)) (cong inr (push a))
         ∙ sym (lUnit (push a))
  ...
  ⊔²→⊔→⊔² (inr (inr x)) = refl
  ⊔²→⊔→⊔² (inr (push a i)) j =
    compPath-filler' (push (g a)) (cong inr (push a)) (~ j) i
  ⊔²→⊔→⊔² (push a i) j = push a (j ∧ i)

  eraseMidIso : Iso (Pushout {C = Pushout g h} f inl)
                    (Pushout (f ∘ g) h)
  fun eraseMidIso = ⊔²→⊔
  inv eraseMidIso = ⊔→⊔²
  rightInv eraseMidIso = ⊔→⊔²→⊔
  leftInv eraseMidIso = ⊔²→⊔→⊔²

module _ {ℓA ℓB ℓC ℓD : Level}
         {A : Type ℓA} {B : Type ℓB}
         {C : Type ℓC} {D : Type ℓD}
         (f : A → B) (g : A → C) (is : Iso B D) where
  private
    HA : HAEquiv B D
    HA = iso→HAEquiv is

    rinv : (d : D) → Iso.fun is (Iso.inv is d) ≡ d
    rinv = isHAEquiv.rinv (snd HA)

  Push→PushSwapL : Pushout f g → Pushout (fun is ∘ f) g
  Push→PushSwapL (inl x) = inl (fun is x)
  Push→PushSwapL (inr x) = inr x
  Push→PushSwapL (push a i) = push a i

  PushSwapL→Push : Pushout (fun is ∘ f) g → Pushout f g
  PushSwapL→Push (inl x) = inl (inv is x)
  PushSwapL→Push (inr x) = inr x
  PushSwapL→Push (push a i) = (cong inl (leftInv is (f a)) ∙ push a) i

  IsoPushSwapL : Iso (Pushout f g) (Pushout (fun is ∘ f) g)
  fun IsoPushSwapL = Push→PushSwapL
  inv IsoPushSwapL = PushSwapL→Push
  rightInv IsoPushSwapL (inl x) j = inl (rinv x j)
  rightInv IsoPushSwapL (inr x) j = inr x
  rightInv IsoPushSwapL (push a i) j = help j i
    where
    help : PathP (λ i → inl (rinv (fun is (f a)) i) ≡ inr (g a))
                 (cong Push→PushSwapL (cong inl (leftInv is (f a)) ∙ push a)) (push a)
    help = cong-∙ Push→PushSwapL (cong inl (leftInv is (f a))) (push a)
         ◁ (λ i j → hcomp (λ k → λ { (i = i1) → push a (j ∧ k)
                                     ; (j = i0) → inl (rinv (fun is (f a)) i)
                                     ; (j = i1) → push a k})
                          (hcomp (λ k → λ { (i = i1) → inl (fun is (f a))
                                     ; (i = i0) → inl (fun is (leftInv is (f a) (j ∧ k)))
                                     ; (j = i0) → inl (rinv (fun is (f a)) i)
                                     ; (j = i1) → inl (fun is (leftInv is (f a) (i ∨ k)))})
                                 (inl (isHAEquiv.com (snd HA) (f a) (~ j) i))))
  leftInv IsoPushSwapL (inl x) i = inl (leftInv is x i)
  leftInv IsoPushSwapL (inr x) i = inr x
  leftInv IsoPushSwapL (push a i) j =
    compPath-filler' (cong inl (leftInv is (f a))) (push a) (~ j) i
