...
-- --   test18 : cong (cong funInv) (rCancel (merid pt)) ≡ congFunct funInv (merid pt) (sym (merid pt)) ∙ rCancel (cong funInv (merid pt)) -- congFunct {!cong!} {!!} {!!} ∙ rCancel {!!}
-- --   test18 = {!!}
...
-- --             {!!}
-- --  -}
-- --   isoHelperPre : (x : S¹ × A) (v : (S¹ , base) wedge (A , pt)) → {!!}
-- --   isoHelperPre = {!!}

-- --   isoHelper : (x : S¹ × A) (v : (S¹ , base) wedge (A , pt)) (q :  inr x ≡ inl tt) (i : I) → (funInv (fun (q i)) ≡ q i) ≡ (funInv (fun ((q ∙ push v) i)) ≡ (q ∙ push v) i)
...
-- --                                     tt (sym (push a))
-- --     where
-- --     test3 : (a₁ : (S¹ , base) wedge (A , pt)) → (q : inr (i∧ a) ≡ inl tt) → Iso ((funInv (fun (q (~ i)))) ≡ q (~ i)) (funInv (fun ((q ∙ push a₁) (~ i))) ≡ (q ∙ push a₁) (~ i))
-- --     test3 (inl x) q = {!!}
-- --     test3 (inr x) q = {!!}
-- --     test3 (push a i) q = {!!}
...
-- --     theHComp : (i j z : I) → (S¹ , base) smash (A , pt)
-- --     theHComp i j z = {!!}
