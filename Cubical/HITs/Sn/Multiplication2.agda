{-# OPTIONS --safe #-}
module Cubical.HITs.Sn.Multiplication2 where
...
invSusp' : ∀ {ℓ} (A : Pointed ℓ) → Susp (typ A) → Susp (typ A)
invSusp' A north = north
invSusp' A south = north
invSusp' A (merid a i) = toSusp A a (~ i)

σ∙ : {n : ℕ} → σS (ptSn n) ≡ refl
σ∙ {n = n} = {!nvSusp'!}
...
inv∧ : ∀ {ℓ} {A B : Pointed ℓ} → (Susp∙ (typ A)) ⋀∙ B →∙ ((Susp∙ (typ A)) ⋀∙ B)
inv∧ {A = A} = (invSusp' A , refl) ⋀→∙ idfun∙ _

liftL : ∀ {ℓ} {A B C : Pointed ℓ} → A ⋀∙ B →∙ C
  → (Susp∙ (typ A)) ⋀∙ B →∙ (Susp∙ (typ C))
fst (liftL f) (inl x) = north
fst (liftL f) (inr (north , y)) = north
fst (liftL f) (inr (south , y)) = north
fst (liftL {C = C} f) (inr (merid a i , y)) =
  toSusp C (fst f (inr (a , y))) i
fst (liftL f) (push (inl north) i) = north
fst (liftL f) (push (inl south) i) = north
fst (liftL {C = C} f) (push (inl (merid a i)) j) =
  (cong (σ C) (cong (fst f) (sym (push (inl a))) ∙ snd f)
  ∙ rCancel (merid (pt C))) (~ j) i
fst (liftL f) (push (inr x) i) = north
fst (liftL f) (push (push a i₁) i) = north
snd (liftL f) = refl

Susp→Id : ∀ {ℓ ℓ'} {A : Pointed ℓ} {B : Pointed ℓ'}
  (f g : A →∙ B) → f ≡ g
Susp→Id = {!!}


inv-comm : ∀ {ℓ} {A B C : Pointed ℓ} (f : (Susp∙ (typ A)) ⋀∙ B →∙ C)
  → ((invSusp' C , refl) ∘∙ liftL f) -- liftL ((invSusp' C , refl) ∘∙ f)
   ≡ liftL (f ∘∙ inv∧ {A = A})
fst (inv-comm f i) (inl x) = north
fst (inv-comm f i) (inr (north , y)) = north
fst (inv-comm f i) (inr (south , y)) = north
fst (inv-comm {C = C} f i) (inr (merid a j , y)) = {!!}
  where
  help : cong (invSusp' C) (toSusp C (fst f (inr (a , y))))
       ≡ {!!}
  help = {!!}
fst (inv-comm f i) (push (inl north) j) = north
fst (inv-comm f i) (push (inl south) j) = north
fst (inv-comm f i) (push (inl (merid a j)) k) = {!!}
fst (inv-comm f i) (push (inr x) i₁) = north
fst (inv-comm f i) (push (push a i₂) i₁) = north
snd (inv-comm f i) j = rUnit (λ _ → north) (~ i) j

sphereFun↑∧ : {n m k : ℕ}
  → (f : S₊∙ n ⋀∙ S₊∙ m →∙ S₊∙ k)
  → S₊∙ (suc n) ⋀∙ S₊∙ m →∙ S₊∙ (suc k)
fst (sphereFun↑∧ {n = zero} {m = m} f) (inl x) = ptSn _
fst (sphereFun↑∧ {n = zero} {m = m} f) (inr (base , y)) = ptSn _
fst (sphereFun↑∧ {n = zero} {m = m} f) (inr (loop i , y)) = σS (fst f (inr (false , y))) i
fst (sphereFun↑∧ {n = zero} {m = m} f) (push (inl base) i) = ptSn _
fst (sphereFun↑∧ {n = zero} {m = m} f) (push (inl (loop i₁)) i) =
  (cong σS (cong (fst f) (sym (push (inl false))) ∙ snd f) ∙ σ∙) (~ i) i₁
fst (sphereFun↑∧ {n = zero} {m = m} f) (push (inr x) i) = ptSn _
fst (sphereFun↑∧ {n = zero} {m = m} f) (push (push a i₁) i) = ptSn _
snd (sphereFun↑∧ {n = zero} {m = m} f) = refl
fst (sphereFun↑∧ {n = suc n} {m = m} f) (inl x) = ptSn _
fst (sphereFun↑∧ {n = suc n} {m = m} f) (inr (north , y)) = ptSn _
fst (sphereFun↑∧ {n = suc n} {m = m} f) (inr (south , y)) = ptSn _
fst (sphereFun↑∧ {n = suc n} {m = m} f) (inr (merid a i , y)) = σS (fst f (inr (a , y))) i
fst (sphereFun↑∧ {n = suc n} {m = m} f) (push (inl north) i) = ptSn _
fst (sphereFun↑∧ {n = suc n} {m = m} f) (push (inl south) i) = ptSn _
fst (sphereFun↑∧ {n = suc n} {m = m} f) (push (inl (merid a i₁)) i) =
  (cong σS (cong (fst f) (sym (push (inl a))) ∙ snd f) ∙ σ∙) (~ i) i₁
fst (sphereFun↑∧ {n = suc n} {m = m} f) (push (inr x) i) = ptSn _
fst (sphereFun↑∧ {n = suc n} {m = m} f) (push (push a i₁) i) = ptSn _
snd (sphereFun↑∧ {n = suc n} {m = m} f) = refl

-- ⋀S : (n m : ℕ) → (S₊∙ n ⋀∙ S₊∙ m) →∙ (S₊∙ (n + m))
-- fst (⋀S zero m) (inl x) = ptSn m
-- fst (⋀S zero m) (inr (false , y)) = y
-- fst (⋀S zero m) (inr (true , y)) = ptSn m
-- fst (⋀S zero m) (push (inl false) i) = ptSn m
-- fst (⋀S zero m) (push (inl true) i) = ptSn m
-- fst (⋀S zero m) (push (inr x) i) = ptSn m
-- fst (⋀S zero m) (push (push a i₁) i) = ptSn m
-- snd (⋀S zero m) = refl
-- ⋀S (suc n) m = sphereFun↑∧ (⋀S n m)
