...
isModalFam : ∀ {ℓ ℓ′} {A : Type ℓ} (B : A → Type ℓ′) → Type (ℓ-max ℓ ℓ′)
isModalFam {_} {_} {A} B = (x : A) → isModal (B x)

module _ {ℓ ℓ′} {A : Type ℓ} {B : ◯ A → Type ℓ′} (B-mod : isModalFam B) (f : (x : A) → B (η x)) where
  postulate
    ◯-ind : ([x] : ◯ A) → B [x]
    ◯-ind-β : (x : A) → ◯-ind (η x) ≡ f x
