...
unique-unfold : ∀ {ℓ} {S : Container {ℓ}} -> (X,ρ : Final {S = S}) -> (C,γ : Coalg₀ {S = S}) -> (_⇒_ {S = S} (C,γ) (X,ρ .fst))  -- unique function into final coalg
unique-unfold X,ρ C,γ = X,ρ .snd C,γ .fst

unique-unfold-function : ∀ {ℓ} {S : Container {ℓ}} -> (X,ρ : Final {S = S}) -> (C,γ : Coalg₀ {S = S}) -> (C,γ .fst) -> (X,ρ .fst .fst)  -- unique function into final coalg
unique-unfold-function X,ρ C,γ y = (unique-unfold X,ρ C,γ) .fst y
...
lemma11 : ∀ {ℓ} {S : Container {ℓ}} (ρ : (n : ℕ) -> X (sequence S) n -> X (sequence S) (suc n)) -> M-type S ≡ X (sequence S) 0
lemma11 {ℓ} {S = S} ρ =
    M-type S
